                                     ...
                                     XsLilError *error)
{
       ...
       equals < line + length && *equals == '=')
    {
        ...
        XsLilStatus u16_status = xs_lil_parse_const_u16(
            block, result_type, operation, (size_t)(line + length - operation), result, &matched_u16, error);
        if(matched_u16)
            return u16_status;
        ...
        if((size_t)(line + length - operation) >= 5U && strncmp(operation, "call ", 5) == 0)
            return parse_call(parser, block, operation, (size_t)(line + length - operation), &result_type, result,
                              error);
...
static XsLilStatus parse_function_body(Parser *parser, XsLilFunction *function, XsLilError *error)
{
    ...
    while(next_line(parser, &line, &length))
    {
        ...
        if(trimmed_length > 4 && trimmed[0] == 'b' && trimmed[1] == 'b')
        {
            ...
            if(label == nullptr)
                return xs_lil_set_error(error, XS_LIL_ALLOCATION_FAILED,
                                        "out of memory while parsing XLIL block label");
    ...
    for(size_t i = 0; i < branch_count; ++i)
    {
        XsLilStatus status = branches[i].is_conditional
                                 ? xs_lil_block_set_branch_if(branches[i].block, branches[i].condition,
                                                              branches[i].target, branches[i].else_target, error)
                                     ...
                                     XsLilError *error)
{
    ...
    *module = nullptr;
    ...
    while(next_line(&parser, &line, &line_length))
    {
        ...
        status = is_definition
                     ? xs_lil_module_add_function_definition(result, name, return_type, parameters, parameter_count,
                                                             &function, error)
