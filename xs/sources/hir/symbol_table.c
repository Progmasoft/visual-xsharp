...
static bool collect_declaration_with_visibility(const XsSyntaxNode *node, const char *module_name,
                                                const char *namespace_name, XsSyntaxVisibility visibility,
                                                XsHirSymbolTable *table, XsDiagnostics *diagnostics)
{
    ...
    const XsHirSymbol *duplicate =
        kind == XS_HIR_SYMBOL_FUNCTION ? find_duplicate_function(table, namespace_name, name, node) : previous;
                                 ...
                                 bool public_namespace, XsHirSymbolTable *table, XsDiagnostics *diagnostics)
{
    ...
    for(size_t i = 0; i < node->child_count; ++i)
    {
        ...
        if(!collect_declaration_with_visibility(child, module_name, namespace_name, child_visibility, table,
                                                diagnostics))
            return false;
    }
    return true;
...
static bool collect_namespace_block(const XsSyntaxNode *declaration, const char *module_name,
                                    const char *parent_namespace, bool public_namespace, XsHirSymbolTable *table,
                                    XsDiagnostics *diagnostics)
{
    ...
    for(size_t i = 0; i < declaration->child_count; ++i)
    {
        ...
        if(child->kind == XS_SYNTAX_PATH || child->kind == XS_SYNTAX_VISIBILITY ||
           child->kind == XS_SYNTAX_ATTRIBUTE_LIST)
            continue;
        if(child->kind == XS_SYNTAX_DECL_NAMESPACE)
        {
            success =
                (child->flags & XS_SYNTAX_FLAG_BLOCK_NAMESPACE) != 0 &&
            ...
            success = collect_extern_block(child, module_name, namespace_name, public_namespace, table, diagnostics) &&
                      success;
        else
            success = collect_declaration(child, module_name, namespace_name, public_namespace, table, diagnostics) &&
                      success;
    }
    free(namespace_name);
    return success;
                                               ...
                                               XsDiagnostics *diagnostics)
{
    ...
    for(size_t i = 0; i < expanded.count; ++i)
    {
        ...
        if(child->kind == XS_SYNTAX_DECL_NAMESPACE)
        {
            ...
            {
                if(!collect_namespace_block(child, module_name, current_namespace, public_namespace, table,
                                            diagnostics))
                    break;
                continue;
            }
