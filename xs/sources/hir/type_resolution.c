...
{
    char message[512];
    snprintf(message, sizeof(message), "generic type '%s' expects %zu type argument(s), got %zu", path, expected,
             actual);
    return xs_diagnostics_add(diagnostics, XS_DIAGNOSTIC_ERROR, node_span(type), message);
}
                               ...
                               const XsHirSymbol **resolved)
{
    ...
    if(standard != nullptr)
    {
        ...
        if(check_arity && standard->max_arity != 0)
            success =
                report_generic_arity_range(diagnostics, type, name, standard->min_arity, standard->max_arity, 0) &&
                success;
        free(name);
        return success;
    }
                                      ...
                                      const XsHirImportScope *import, XsDiagnostics *diagnostics)
{
    ...
    if(standard != nullptr)
    {
        ...
        if(!standard_type_arity_ok(standard, actual))
            success =
                report_generic_arity_range(diagnostics, type, name, standard->min_arity, standard->max_arity, actual) &&
                success;
                                    ...
                                    const XsHirImportScope *import, XsDiagnostics *diagnostics)
{
    if(type->kind == XS_SYNTAX_TYPE_GENERIC)
    {
        ...
        return resolve_generic_type_node(type, namespace_name, current_file_id, generics, symbols, import,
                                         diagnostics) &&
               success;
    }
    if(type->kind != XS_SYNTAX_TYPE_NAMED)
    ...
    if(name == nullptr)
        return false;
    const XsHirSymbol *symbol = nullptr;
    bool success = resolve_named_type(type, namespace_name, generics, symbols, import, diagnostics, current_file_id,
                                      true, &symbol);
                                      ...
                                      const XsMacroStatementExpansionSet *macro_statements)
{
    ...
    if(node->kind == XS_SYNTAX_TYPE_NAMED || node->kind == XS_SYNTAX_TYPE_GENERIC ||
       ...
       node->kind == XS_SYNTAX_TYPE_MAP || node->kind == XS_SYNTAX_TYPE_POINTER ||
