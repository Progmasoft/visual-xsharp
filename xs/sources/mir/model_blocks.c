...
XsMirStatus xs_mir_block_add_const_i64(XsMirBlock *block, int64_t value, XsMirValueId *result, XsMirError *error)
{
    ...
    XsMirStatus status =
        xs_mir_function_add_value(block->owner, (XsMirType){.kind = XS_LIL_TYPE_I64}, &value_id, error);
    if(status != XS_MIR_OK)
        return status;
...
XsMirStatus xs_mir_block_add_const_i32(XsMirBlock *block, int32_t value, XsMirValueId *result, XsMirError *error)
{
    ...
    XsMirValueId value_id = 0;
    XsMirStatus status =
        xs_mir_function_add_value(block->owner, (XsMirType){.kind = XS_LIL_TYPE_I32}, &value_id, error);
    if(status != XS_MIR_OK)
        return status;
    status = append_instruction(block,
                                (XsMirInstruction){
...
XsMirStatus xs_mir_block_add_const_bool(XsMirBlock *block, bool value, XsMirValueId *result, XsMirError *error)
{
    ...
    XsMirValueId value_id = 0;
    XsMirStatus status =
        xs_mir_function_add_value(block->owner, (XsMirType){.kind = XS_LIL_TYPE_BOOL}, &value_id, error);
    if(status != XS_MIR_OK)
        return status;
    status = append_instruction(block,
                                (XsMirInstruction){
