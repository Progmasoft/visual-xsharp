...
public:
    ...
    TemporaryRegistry &operator=(const TemporaryRegistry &) = delete;
    ~TemporaryRegistry()
    {
...
[[nodiscard]] std::filesystem::path JavaProgram()
{
...
[[nodiscard]] std::string ProjectEvaluatorClasspath()
{
...
#ifdef XS_PROJECT_EVALUATOR_CLASSPATH_BUILD
    {
        const std::filesystem::path buildLibraryDirectory =
            std::filesystem::path(XS_PROJECT_EVALUATOR_CLASSPATH_BUILD).parent_path();
        std::error_code error;
...
[[nodiscard]] int RunProgram(const std::filesystem::path &program, std::span<const std::string> arguments)
{
#ifdef _WIN32
...
[[nodiscard]] int RunProjectEvaluator(std::span<const std::string> commandArguments)
{
...
[[nodiscard]] bool RunResolver(std::string_view mode, const std::filesystem::path &output)
{
    const std::vector<std::string> arguments{std::string(mode), ".", output.string()};
    return RunProjectEvaluator(arguments) == 0;
}

[[nodiscard]] std::optional<std::vector<char>> ReadRegistry(const std::filesystem::path &path)
{
...
[[nodiscard]] std::optional<std::vector<std::string_view>> SplitRecords(const std::vector<char> &bytes)
{
...
{
public:
    explicit RecordReader(std::span<const std::string_view> records) : records_(records) {}
...
[[nodiscard]] std::optional<XsBuildOutput> ParseOutput(std::string_view text)
{
...
[[nodiscard]] bool ParseCompilerHeader(RecordReader &reader, ResolvedProject &project)
{
    ...
    if(!entry || entry->empty() || !compilerVersion || !standard || backend != "llvm" ||
       (buildMode != "debug" && buildMode != "release") || !output || !warning)
        return false;
    ...
    if(!warningFound || !warningsAsErrors || !experimental || !shadow || !undefined || !typeSafeFormat || !xpp ||
       !xmm || !optLevel || !llvmCompiler || !lto)
        return false;
...
[[nodiscard]] std::optional<ResolvedProject> ParseRegistry(const std::vector<char> &bytes, bool requireSources)
{
...
std::optional<ResolvedProject> ResolveProject(bool requireSources)
{
