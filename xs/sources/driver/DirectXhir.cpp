...
bool xs_driver_build_direct_xhir(const char *input_path, const char *text, std::size_t length)
{
    ...
    {
        std::fprintf(stderr, "vxs: XHIR compiler core session could not be created (status %u)\n",
    ...
    if(success)
        ...
        std::fputs("vxs: XHIR compiler core did not produce a verified XLIL module\n", stderr);
