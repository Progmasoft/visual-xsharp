...
module Visual.XSharp.Compiler
    ( CompilerInput (..), FrontendArtifacts (..), compileToCorePrep, compileEntryToCorePrep, validateEntryPoint ) where
...
data FrontendArtifacts = FrontendArtifacts
    { artifactParsedAST :: ParsedAST, artifactRenamedAST :: RenamedAST, artifactResolvedAST :: ResolvedAST
    , artifactTypedAST :: TypedAST, artifactCore :: CoreModule, artifactOptimizedCore :: CoreModule
...
validateEntryPoint (QualifiedName parts) (TypedAST (SyntaxTree namespace declarations)) =
    let className = last parts
        ...
        matching = [declaration | declaration@TypeDeclaration{} <- declarations, resolvedSpelling (declarationName declaration) == className]
...
validateEntryMethod TypeDeclaration{typeMembers = members} =
    case [member | member@FunctionDeclaration{} <- members, identifierText (resolvedSpelling (declarationName member)) == "Main"] of
        ...
        method:_
            ...
            | declarationReturnSyntax method /= ExplicitType (Identifier "unit") -> failure method "VXE0008" "entry Main must return void"
            | otherwise -> Right ()
  where failure method code message = Left [entryProblem code (Just (declarationSpan method)) message]
