...
module Visual.XSharp.Parser
    ( TokenKind (..), Token (..), ParserInput (..), Parser (..), defaultParser, runParser ) where
...
parseMember = do
    ...
    _ <- symbol ")"
    body <- parseBlock
    pure (FunctionDeclaration (mergeSpan nameSpan (blockSpan body nameSpan)) name () returnType parameters body isStatic access)
...
parseAccess = do
    ...
    pure $ case fmap tokenText token of
        Just "public" -> PublicAccess; Just "internal" -> InternalAccess; Just "private" -> PrivateAccess
        Just "protected" -> ProtectedAccess; _ -> DefaultAccess

manyUntil :: String -> P a -> P [a]
manyUntil closing parser = do
    done <- peekText closing
    eof <- peekKind EndOfFileToken
    if done then pure [] else if eof then failCurrent "VXP0002" "unterminated declaration" else (:) <$> parser <*> manyUntil closing parser
...
parseTypeSyntax = do
    ...
    pure $ case tokenText token of "auto" -> AutoType; "void" -> ExplicitType (Identifier "unit"); value -> ExplicitType (Identifier value)

parseBlock :: P (Block Identifier ())
parseBlock = do _ <- symbol "{"; statements <- go; _ <- symbol "}"; pure (Block statements)
  where
    go = do
        done <- peekText "}"; eof <- peekKind EndOfFileToken
        if done then pure [] else if eof then failCurrent "VXP0002" "unterminated block" else (:) <$> parseStatement <*> go
...
parseReturn = do
    start <- keyword "return"; empty <- peekText ";"
    if empty then do _ <- symbol ";"; pure (ReturnStatement (tokenSpan start) Nothing)
    else do value <- parseExpression; end <- optionalSemicolon
            pure (ReturnStatement (mergeSpan (tokenSpan start) (maybe (expressionSpan value) tokenSpan end)) (Just value))

parseIf :: P (Statement Identifier ())
parseIf = do
    start <- keyword "if"; hasParen <- optionalSymbol "("; condition <- parseExpression
    if hasParen then do _ <- symbol ")"; pure () else pure ()
    trueBlock <- parseBlock; falseBlock <- optionalParser (keyword "else" >> parseBlock)
    pure (IfStatement (mergeSpan (tokenSpan start) (blockSpan (maybe trueBlock id falseBlock) (tokenSpan start))) condition trueBlock falseBlock)
...
parseBinding = do
    ...
    pure (BindingStatement (mergeSpan startSpan (maybe (expressionSpan value) tokenSpan end)) kind bindingType name () value)

parseAssignmentOrExpression :: P (Statement Identifier ())
parseAssignmentOrExpression = do
    expression <- parseExpression; assignment <- optionalSymbol "="
    if assignment then case expression of
...
parseLogicalOr, parseLogicalAnd, parseEquality, parseComparison, parseAdditive, parseMultiplicative :: P (Expression Identifier ())
  ...
  where
    continue left = do
        ...
        case next >>= (\token -> lookup (tokenText token) operators) of
            Nothing -> pure left
            Just operator -> do _ <- takeToken; right <- operand
...
parseUnary = do
    next <- peekToken
    case next >>= (\token -> lookup (tokenText token) [("+", UnaryPlus), ("-", UnaryNegate), ("!", LogicalNot), ("not", LogicalNot)]) of
        Just operator -> do start <- takeToken; value <- parseUnary
...
parsePostfix = parsePrimary >>= calls
  where
    calls callee = do
        call <- optionalSymbol "("
        if not call then pure callee else do
            arguments <- separated "," parseExpression; close <- symbol ")"
...
parsePrimary = do
    next <- peekToken
    case next of
        Just token | tokenKind token == IntegerToken -> do _ <- takeToken; pure (LiteralExpression (tokenSpan token) (IntegerLiteral (read (filter (/= '_') (tokenText token)))) ())
        ...
        Just token | tokenText token == "(" -> do
            open <- takeToken; unit <- peekText ")"
            if unit then do close <- symbol ")"; pure (LiteralExpression (mergeSpan (tokenSpan open) (tokenSpan close)) UnitLiteral ())
...
expressionSpan expression = case expression of
    NameExpression value _ _ -> value; LiteralExpression value _ _ -> value; CallExpression value _ _ _ -> value
    UnaryExpression value _ _ _ -> value; BinaryExpression value _ _ _ _ -> value
...
identifier = do token <- satisfy ((== IdentifierToken) . tokenKind) "identifier"; pure (Identifier (tokenText token), tokenSpan token)
...
optionalSemicolon :: P (Maybe Token)
optionalSemicolon = do present <- peekText ";"; if present then Just <$> takeToken else pure Nothing
...
takeToken :: P Token
takeToken = P $ \tokens -> case tokens of token:rest -> Right (token, rest); [] -> Left (Diagnostic ParserStage Error "VXP0008" Nothing "unexpected end of input")
