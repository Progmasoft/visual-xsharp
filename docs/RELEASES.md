...
- `0.2.8` is the XLIL v1 parser compatibility hotfix for exact-width `i64` bit patterns emitted by the canonical writer.
...
- `0.2.5` adds the C++23 Preview DIMCLI command schema, reusable MIR control-flow/dataflow analysis, a verified optimization
  pipeline, and feature-gated Kotlin dependencies with the version-1 SQLite lock format.
- `0.2.3` separates the language-neutral X Platform runtime from the Visual X# compiler, reserves the XPLR project settings,
  and keeps LLVM as the only implemented Visual X# backend.
- `0.2.2` added the experimental collector topology that has since moved into the independent X Platform project as XPG.
...
- `0.2.0` makes runtime-sized `[T]` arrays explicit across XHIR, XMIR, XLIL, the public C23 API, and LLVM. Construction,
  checked access and mutation, count queries, same-module calls, and `for` iteration reach native `.vxse` output.
- `0.1.9` carries arbitrary nominal-value field projections through typed XHIR, MIR/XLIL aggregate extraction, LLVM IR,
  and native `.vxse` execution. It also establishes the first public strict-C23 trait/implementation helper headers and
...
- `0.1.8` establishes the target-independent XGC region/mark/card/SATB/root metadata core and transfers the optional
  whole-program mode through Kotlin project resolution and CLI policy. Native `.vxse` compilation remains the primary
...
- `0.1.7` preserves built-in fixed arrays as first-class typed HIR/MIR/XLIL values, exposes their registry through the
  public C23 XLIL API, and lowers fixed-array construction and constant indexing through LLVM to native `.vxse` output.
- `0.1.6` carries every fixed-width integer operation through typed HIR, verified and optimized MIR, XLIL, signed or
  unsigned LLVM lowering, object emission, and native `.vxse` execution. The public C23 XLIL API exposes the same typed
  operation model.
...
- `0.1.4` adds Rust compiler-core statement-match CFG lowering and explicit MIR local storage that reaches XLIL stack
  slots, LLVM memory operations, and native `.vxse` execution. It also adds post-test `do`/`while` sugar and distinct
...
- `0.1.2` connects supported compiler-core sessions back to native emission. Typed Rust HIR lowers through verified and
  optimized MIR into XLIL text, which the public C23 XLIL parser verifies before LLVM IR, object, and `.vxse` emission.
...
- The first `.vxse` target format is Linux ELF; PE comes after ELF support.
