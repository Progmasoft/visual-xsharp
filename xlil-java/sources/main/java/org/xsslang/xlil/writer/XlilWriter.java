...
public final class XlilWriter implements AutoCloseable
{
    ...
    private XlilWriter(String name)
    {
        ...
        int status = LilNative.callInt("xs_lil_module_create", FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, ADDRESS),
                CString.allocate(arena, name), output, error);
        ...
        status = LilNative.callInt("xs_lil_builder_create", FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, ADDRESS),
                module, output, error);
        if(status != LilNative.OK)
        {
            String message = LilNative.errorMessage(error);
    ...
    public XlilType aggregateType(String name, List<XlilType> fields)
    {
        ...
        int status = LilNative.callInt("xs_lil_module_add_aggregate_type",
                FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, ADDRESS, C_SIZE, ADDRESS, ADDRESS), module,
                CString.allocate(arena, name), fieldTypes, (long) fields.size(), output, error);
        check(status);
    ...
    public XlilType fixedArrayType(XlilType elementType, long length)
    {
        ...
        ensureOpen();
        MemorySegment output = arena.allocate(TYPE_LAYOUT);
        int status = LilNative.callInt("xs_lil_module_add_array_type",
                FunctionDescriptor.of(C_INT, ADDRESS, TYPE_LAYOUT, C_INT64, ADDRESS, ADDRESS), module,
                nativeType(elementType), length, output, error);
        check(status);
        return readType(output);
    }
    ...
    public XlilType dynamicArrayType(XlilType elementType)
    {
        ensureOpen();
        MemorySegment output = arena.allocate(TYPE_LAYOUT);
        int status = LilNative.callInt("xs_lil_module_add_dynamic_array_type",
                FunctionDescriptor.of(C_INT, ADDRESS, TYPE_LAYOUT, ADDRESS, ADDRESS), module, nativeType(elementType),
                output, error);
        check(status);
        return readType(output);
    }

    public void declareFunction(String name, XlilType returnType, List<XlilType> parameters)
    {
        ensureOpen();
        Objects.requireNonNull(parameters, "parameters");
        int status = LilNative.callInt("xs_lil_module_add_function",
                FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, TYPE_LAYOUT, ADDRESS, C_SIZE, ADDRESS), module,
                CString.allocate(arena, name), nativeType(returnType), typeArray(parameters), (long) parameters.size(),
                error);
        check(status);
    }

    public XlilFunctionWriter defineFunction(String name, XlilType returnType, List<XlilType> parameters)
    {
        ensureOpen();
        Objects.requireNonNull(parameters, "parameters");
        MemorySegment output = arena.allocate(ADDRESS);
        int status = LilNative.callInt("xs_lil_module_add_function_definition",
                FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, TYPE_LAYOUT, ADDRESS, C_SIZE, ADDRESS, ADDRESS), module,
                CString.allocate(arena, name), nativeType(returnType), typeArray(parameters), (long) parameters.size(),
                output, error);
        check(status);
    ...
    {
        ensureOpen();
        int status = LilNative.callInt("xs_lil_module_verify", FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS), module,
                error);
        check(status);
    }
    ...
    public void close()
    {
        ...
        closed = true;
        LilNative.callVoid("xs_lil_builder_destroy", FunctionDescriptor.ofVoid(ADDRESS), builder);
        LilNative.callVoid("xs_lil_module_destroy", FunctionDescriptor.ofVoid(ADDRESS), module);
        LilNative.errorDestroy(error);
        arena.close();
    }
    ...
    {
        int status = LilNative.callInt("xs_lil_builder_position_at_end",
                FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, ADDRESS), builder, block, error);
        check(status);
    }
