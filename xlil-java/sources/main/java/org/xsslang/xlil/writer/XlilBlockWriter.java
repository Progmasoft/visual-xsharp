...
public final class XlilBlockWriter {
  ...
  public int id() {
    return LilNative.callInt(
        "xs_lil_block_id", FunctionDescriptor.of(C_INT, ADDRESS), block);
  }
  ...
  public XlilValue constF32(float value) {
    return valueResult(
        "xs_lil_builder_const_f32_bits", C_INT, Float.floatToRawIntBits(value));
  }

  public XlilValue constF64(double value) {
    return valueResult(
        "xs_lil_builder_const_f64_bits", C_INT64, Double.doubleToRawLongBits(value));
  }
  ...
  public XlilValue constString(String value, Utf16Encoding encoding) {
    ...
    int status =
        LilNative.callInt(
            "xs_lil_builder_const_str",
            FunctionDescriptor.of(
                C_INT, ADDRESS, C_INT, ADDRESS, C_SIZE, ADDRESS, ADDRESS),
            owner.builder(),
            encoding.ordinal(),
            nativeUnits,
            (long) units.length,
  ...
  public XlilValue integer(
      IntegerOperation operation, XlilValue left, XlilValue right) {
    Objects.requireNonNull(operation, "operation");
    return binaryResult(
        "xs_lil_builder_binary_integer", operation.ordinal(), left, right);
  }

  public XlilValue floating(
      FloatOperation operation, XlilValue left, XlilValue right) {
    Objects.requireNonNull(operation, "operation");
    return binaryResult(
        "xs_lil_builder_binary_float", operation.ordinal(), left, right);
  }

  public XlilValue compareFloat(
      FloatComparison operation, XlilValue left, XlilValue right) {
    Objects.requireNonNull(operation, "operation");
    return binaryResult(
        "xs_lil_builder_compare_float", operation.ordinal(), left, right);
  }

  public XlilValue compareString(
      StringComparison operation, XlilValue left, XlilValue right) {
    Objects.requireNonNull(operation, "operation");
    return binaryResult(
        "xs_lil_builder_compare_str", operation.ordinal(), left, right);
  }
  ...
  public XlilValue not(XlilValue operand) {
    ...
    int status =
        LilNative.callInt(
            "xs_lil_builder_not_bool",
            FunctionDescriptor.of(C_INT, ADDRESS, C_INT, ADDRESS, ADDRESS),
            owner.builder(),
            operand.id(),
            output,
            owner.error());
  ...
  public XlilValue arraySet(
      XlilValue array, XlilValue index, XlilValue replacement) {
    position();
    MemorySegment output = owner.arena().allocate(C_INT);
    int status =
        LilNative.callInt(
            "xs_lil_builder_array_set",
            FunctionDescriptor.of(
                C_INT, ADDRESS, C_INT, C_INT, C_INT, ADDRESS, ADDRESS),
            owner.builder(),
            array.id(),
            index.id(),
            replacement.id(),
  ...
  public void store(XlilSlot slot, XlilValue value) {
    ...
    int status =
        LilNative.callInt(
            "xs_lil_builder_store",
            FunctionDescriptor.of(C_INT, ADDRESS, C_INT, C_INT, ADDRESS),
            owner.builder(),
            slot.id(),
            value.id(),
            owner.error());
  ...
  public void branchIf(
      XlilValue condition, XlilBlockWriter thenBlock, XlilBlockWriter elseBlock) {
    position();
    int status =
        LilNative.callInt(
            "xs_lil_builder_branch_if",
            FunctionDescriptor.of(
                C_INT, ADDRESS, C_INT, ADDRESS, ADDRESS, ADDRESS),
            owner.builder(),
            condition.id(),
            thenBlock.block,
            elseBlock.block,
  ...
  private XlilValue valueResult(
      String function, java.lang.foreign.MemoryLayout valueLayout, Object value) {
    position();
    MemorySegment output = owner.arena().allocate(C_INT);
    int status =
        LilNative.callInt(
            function,
            FunctionDescriptor.of(
                C_INT, ADDRESS, valueLayout, ADDRESS, ADDRESS),
            owner.builder(),
            value,
            output,
            owner.error());
  ...
  private XlilValue binaryResult(
      String function, int operation, XlilValue left, XlilValue right) {
    position();
    MemorySegment output = owner.arena().allocate(C_INT);
    int status =
        LilNative.callInt(
  ...
  private XlilValue callNative(
      String callee, List<XlilValue> arguments, boolean expectVoid) {
    ...
    int status =
        LilNative.callInt(
            ...
            owner.builder(),
            org.xsslang.ffi.c.CString.allocate(owner.arena(), callee),
            nativeArguments,
            (long) arguments.size(),
            output,
            owner.error());
  ...
  private XlilValue composite(
      String function, XlilType type, List<XlilValue> values) {
    ...
    int status =
        LilNative.callInt(
            function,
            FunctionDescriptor.of(
                C_INT, ADDRESS, TYPE_LAYOUT, ADDRESS, C_SIZE, ADDRESS, ADDRESS),
            owner.builder(),
  ...
  private XlilValue twoIdResult(String function, int first, int second) {
    ...
    int status =
        LilNative.callInt(
            function,
            FunctionDescriptor.of(
                C_INT, ADDRESS, C_INT, C_INT, ADDRESS, ADDRESS),
            owner.builder(),
            first,
            second,
            output,
