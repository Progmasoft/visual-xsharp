...
public final class XlilFunctionWriter
{
    ...
    public XlilSlot slot(XlilType type)
    {
        ...
        int status = LilNative.callInt("xs_lil_function_add_slot",
                FunctionDescriptor.of(C_INT, ADDRESS, TYPE_LAYOUT, ADDRESS, ADDRESS), function, owner.nativeType(type),
                output, owner.error());
        owner.check(status);
    ...
    public XlilBlockWriter block(String label)
    {
        ...
        int status = LilNative.callInt("xs_lil_builder_append_block",
                FunctionDescriptor.of(C_INT, ADDRESS, ADDRESS, ADDRESS, ADDRESS, ADDRESS), owner.builder(), function,
                org.xsslang.ffi.c.CString.allocate(owner.arena(), label), output, owner.error());
        owner.check(status);
