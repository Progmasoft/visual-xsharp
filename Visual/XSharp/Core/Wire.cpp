...
{
public:
    Reader(std::span<const std::uint8_t> bytes, const Limits &limits) : bytes_(bytes), limits_(limits) {}

    [[nodiscard]] auto Document() -> DecodeResult
    {
        ...
        module.functions =
            Vector<Function>(limits_.maximumFunctions, "function count", [this] { return ReadFunction(); });
...
private:
    ...
    [[nodiscard]] auto Result() -> DecodeResult
    {
        return DecodeResult{std::move(module_), std::move(error_)};
    }
    [[nodiscard]] auto Failure(ErrorKind kind, std::string context, std::string message) -> DecodeResult
    {
    ...
    [[nodiscard]] auto Byte(std::string_view context) -> std::uint8_t
    {
    ...
    template <typename Integer> [[nodiscard]] auto Unsigned(std::string_view context) -> Integer
    {
    ...
    [[nodiscard]] auto Count(std::size_t maximum, std::string_view context) -> std::size_t
    {
    ...
    [[nodiscard]] auto Vector(std::size_t maximum, std::string_view context, Decode decode) -> std::vector<Value>
    {
    ...
    [[nodiscard]] auto Text(std::string_view context) -> std::u32string
    {
    ...
    [[nodiscard]] auto QualifiedName(std::string_view context) -> std::vector<std::u32string>
    {
        return Vector<std::u32string>(65535U, context, [this, context] { return Text(context); });
    }
    [[nodiscard]] auto Symbol(std::string_view context) -> SymbolName
    {
    ...
    [[nodiscard]] auto ReadType(std::size_t depth = 0U) -> Type
    {
        ...
        case 4:
            return Type::named(QualifiedName("named type"),
                               Vector<Type>(limits_.maximumOperands, "type argument count",
                                            [this, depth] { return ReadType(depth + 1U); }));
        case 5:
        {
            auto parameters = Vector<Type>(limits_.maximumParameters, "function type parameter count",
                                           [this, depth] { return ReadType(depth + 1U); });
    ...
    [[nodiscard]] auto Boolean(std::string_view context) -> bool
    {
    ...
    [[nodiscard]] auto ReadLiteral() -> Literal
    {
    ...
    [[nodiscard]] auto ReadExpression(std::size_t depth = 0U) -> Expression
    {
        ...
        case 2:
        {
            ...
            auto arguments = Vector<Expression>(limits_.maximumOperands, "call argument count",
                                                [this, depth] { return ReadExpression(depth + 1U); });
        ...
        case 3:
        {
            ...
            auto arguments = Vector<Expression>(limits_.maximumOperands, "primitive operand count",
                                                [this, depth] { return ReadExpression(depth + 1U); });
            return Expression::InvokePrimitive(static_cast<Primitive>(primitiveTag), std::move(arguments),
                                               std::move(valueType));
        }
        default:
    ...
    [[nodiscard]] auto ReadStatement() -> Statement
    {
        ...
        case 3:
        {
            ...
            auto whenTrue = Vector<Statement>(limits_.maximumStatements, "true branch statement count",
                                              [this] { return ReadStatement(); });
            auto whenFalse = Vector<Statement>(limits_.maximumStatements, "false branch statement count",
                                               [this] { return ReadStatement(); });
    ...
    [[nodiscard]] auto ReadParameter() -> Parameter
    {
        return Parameter{Symbol("parameter symbol"), ReadType()};
    }
    [[nodiscard]] auto ReadFunction() -> Function
    {
        Function function;
        function.symbol = Symbol("function symbol");
        function.parameters =
            Vector<Parameter>(limits_.maximumParameters, "parameter count", [this] { return ReadParameter(); });
        function.returnType = ReadType();
        function.body =
            Vector<Statement>(limits_.maximumStatements, "statement count", [this] { return ReadStatement(); });
        return function;
    }
};
...
public:
    ...
    void Document(const Module &module)
    {
        ...
        Vector(module.functions, limits_.maximumFunctions, "function count",
               [this](const Function &function) { WriteFunction(function); });
    }
...
private:
    ...
    template <typename Integer> void Unsigned(Integer value)
    {
        static_assert(std::is_unsigned_v<Integer>);
        for(std::size_t shift = 0; shift < sizeof(Integer) * 8U; shift += 8U)
    ...
    void QualifiedName(const std::vector<std::u32string> &parts, std::string_view context)
    {
        Vector(parts, 65535U, context, [this, context](const auto &part) { Text(part, context); });
    }
    void Symbol(const SymbolName &symbol, std::string_view context)
    {
        if(symbol.id == 0U)
        {
            Fail(ErrorKind::InvalidSymbol, std::string(context), "symbol id must be positive");
    ...
    void WriteType(const Type &type, std::size_t depth = 0U)
    {
        if(depth > limits_.maximumTypeDepth)
        {
            Fail(ErrorKind::LimitExceeded, "type", "type nesting exceeds configured limit");
        ...
        case Type::Kind::Named:
            ...
            Vector(type.components, limits_.maximumOperands, "type argument count",
                   [this, depth](const Type &argument) { WriteType(argument, depth + 1U); });
            return;
    ...
    void WriteLiteral(const Literal &literal)
    {
    ...
    void WriteExpression(const Expression &expression, std::size_t depth = 0U)
    {
        if(depth > limits_.maximumExpressionDepth)
        {
            Fail(ErrorKind::LimitExceeded, "expression", "expression nesting exceeds configured limit");
        ...
        case Expression::Kind::Apply:
            ...
            Vector(expression.operands, limits_.maximumOperands, "call argument count",
                   [this, depth](const Expression &value) { WriteExpression(value, depth + 1U); });
            return;
        case Expression::Kind::Primitive:
            Vector(expression.operands, limits_.maximumOperands, "primitive operand count",
                   [this, depth](const Expression &value) { WriteExpression(value, depth + 1U); });
            return;
        }
    }
    void WriteStatement(const Statement &statement)
    {
        ...
        case Statement::Kind::If:
            WriteExpression(statement.expression);
            Vector(statement.trueBranch, limits_.maximumStatements, "true branch statement count",
                   [this](const Statement &value) { WriteStatement(value); });
            Vector(statement.falseBranch, limits_.maximumStatements, "false branch statement count",
                   [this](const Statement &value) { WriteStatement(value); });
            return;
    ...
    void WriteFunction(const Function &function)
    {
        ...
        Vector(function.parameters, limits_.maximumParameters, "parameter count",
        ...
        Vector(function.body, limits_.maximumStatements, "statement count",
               [this](const Statement &statement) { WriteStatement(statement); });
    }
};
} // namespace

auto Encode(const Module &module, const Limits &limits) -> EncodeResult
{
...
auto Decode(std::span<const std::uint8_t> bytes, const Limits &limits) -> DecodeResult
{
