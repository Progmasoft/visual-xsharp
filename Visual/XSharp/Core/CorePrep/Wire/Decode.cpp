...
public:
    ...
    [[nodiscard]] auto document() -> DecodeResult
    {
        ...
        module.functions = vector<Function>(limits_.maximum_functions, "function count", [this] { return function(); });
...
private:
    ...
    [[nodiscard]] auto result() -> DecodeResult { return DecodeResult{std::move(module_), std::move(error_)}; }
    ...
    [[nodiscard]] auto unsigned_integer(std::string_view context) -> Integer
    {
    ...
    [[nodiscard]] auto qualified_name(std::string_view context) -> std::vector<std::u32string>
    {
    ...
    [[nodiscard]] auto type(std::size_t depth = 0) -> Type
    {
        ...
        case 2: return Type::int64();
        case 3: return Type::int32();
        case 4: return Type::string();
        case 5:
        {
            auto parameters = vector<Type>(65535U, "function type parameter count", [this, depth] { return type(depth + 1U); });
        ...
        case 6:
        {
            ...
            auto arguments = vector<Type>(65535U, "type argument count", [this, depth] { return type(depth + 1U); });
            return Type::named(std::move(name), std::move(arguments));
        }
        case 7: return Type::type_variable(symbol("type variable symbol"));
        default:
    ...
    [[nodiscard]] auto literal(const Type &value_type) -> Literal
    {
        switch(value_type.kind)
        {
        case Type::Kind::Unit: return std::monostate{};
        case Type::Kind::Bool: return boolean("boolean literal");
        ...
        case Type::Kind::String: return text("string literal");
    ...
    [[nodiscard]] auto operation_tag() -> Operation
    {
    ...
    void operation(Instruction &value)
    {
        value.operation = operation_tag();
        value.operands = vector<Atom>(limits_.maximum_operands_per_instruction, "operand count", [this] { return atom(); });
        if(error_)
            return;
    ...
    [[nodiscard]] auto instruction() -> Instruction
    {
        Instruction value;
        ...
        case Instruction::Kind::Assign:
            ...
            break;
        case Instruction::Kind::Evaluate: operation(value); break;
        }
        return value;
    }

    [[nodiscard]] auto terminator() -> Terminator
    {
        Terminator value;
        ...
        switch(value.kind)
        {
        case Terminator::Kind::Return: value.value = atom(); break;
        case Terminator::Kind::Branch:
            value.value = atom();
            value.true_target = unsigned_integer<BlockId>("true target");
            value.false_target = unsigned_integer<BlockId>("false target");
            break;
        case Terminator::Kind::Jump: value.true_target = unsigned_integer<BlockId>("jump target"); break;
        case Terminator::Kind::Unreachable: break;
        }
        return value;
    }

    [[nodiscard]] auto block() -> Block
    {
        Block value;
        value.id = unsigned_integer<BlockId>("block id");
        value.instructions = vector<Instruction>(limits_.maximum_instructions_per_block, "instruction count",
                                                 [this] { return instruction(); });
        value.terminator = terminator();
        return value;
    }

    [[nodiscard]] auto parameter() -> Parameter { return Parameter{symbol("parameter symbol"), type()}; }

    [[nodiscard]] auto function() -> Function
    {
        Function value;
        value.symbol = symbol("function symbol");
        value.parameters = vector<Parameter>(limits_.maximum_parameters_per_function, "parameter count",
                                             [this] { return parameter(); });
        value.return_type = type();
        value.entry = unsigned_integer<BlockId>("entry block");
        value.blocks = vector<Block>(limits_.maximum_blocks_per_function, "block count", [this] { return block(); });
        return value;
    }
};
} // namespace
