...
namespace Visual::XSharp::Driver
{
    ...
    LinkNativeExecutable(const NativeLinkRequest &request) -> NativeLinkResult
    {
        ...
        // The generated bridge is a freestanding PE entry and currently needs
        // no CRT. Avoiding implicit default libraries keeps the first
        // executable boundary small, deterministic, and independent from Visual
        // Studio's compiler/linker binaries.
        ...
        // Owning strings remain separate from argv pointers. Reserving both
        // vectors prevents growth from invalidating memory passed to
        // `_wspawnvp`.
        ...
        const auto status
            = _wspawnvp(_P_WAIT, program.c_str(), arguments.data());
        ...
        if (status != 0)
            return { static_cast<int>(status),
                     "lld-link failed with exit code "
