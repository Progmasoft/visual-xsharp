...
[[nodiscard]] int ExecuteNative(const std::filesystem::path &executable)
{
...
[[nodiscard]] int LinkObjectInput(const std::filesystem::path &object, bool execute)
{
    ...
    const Visual::XSharp::Driver::NativeLinkRequest request{
        executable, {object}, Visual::XSharp::Backend::LLVM::ObjectFormat::Coff};
                                 ...
                                 const XsEffectiveCompilerOptions &effective)
{
    ...
    const bool built =
        xs_driver_process_core_artifact_as(core.Path().string().c_str(), sourceText.c_str(), options.command, output,
                                           &effective.compiler, effective.target ? effective.target->c_str() : nullptr);
    if(!built)
        return false;
...
[[nodiscard]] int RunFile(const XsCliOptions &options)
{
    ...
    if(options.input == XS_BUILD_INPUT_CORE)
    {
        ...
        const auto output = options.command == XS_CLI_COMMAND_RUN ? XS_BUILD_OUTPUT_BINARY : effective.output;
        if(!xs_driver_process_core_artifact(fileText.c_str(), options.command, output, &effective.compiler,
                                            effective.target ? effective.target->c_str() : nullptr))
            return 1;
...
[[nodiscard]] int RunProject(const XsCliOptions &options)
{
    ...
    if(options.command == XS_CLI_COMMAND_BUILD &&
       (effective.output == XS_BUILD_OUTPUT_OBJECT || effective.output == XS_BUILD_OUTPUT_ASSEMBLY))
    {
    ...
    if(!xs_driver_process_core_artifact_as(corePathText.c_str(), artifactBaseText.c_str(), options.command, output,
                                           &effective.compiler, effective.target ? effective.target->c_str() : nullptr))
        return 1;
...
auto Visual::XSharp::Cli::Run(int argc, char **argv) -> int
{
