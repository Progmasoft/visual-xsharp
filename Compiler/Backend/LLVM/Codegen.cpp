...
struct Generator final
{
    ...
    [[nodiscard]] auto CreateExecutableEntry(const llvm::Triple &triple) -> bool
    {
        ...
        {
            if(candidate.source->symbol.spelling != U"Main" || !candidate.source->parameter_types.empty() ||
               candidate.source->return_type.kind != core::Type::Kind::Unit)
                continue;
            if(entry != nullptr)
            {
                fail(ErrorKind::InvalidEntryPoint, "VXL2008",
                     "native executable contains more than one parameterless void Main function");
                return false;
            }
            entry = &candidate;
        }
        if(entry == nullptr)
        {
            fail(ErrorKind::InvalidEntryPoint, "VXL2007",
                 "native executable requires one parameterless void Main function");
            return false;
        }
...
[[nodiscard]] auto CodeGenerationLevel(OptimizationLevel level) -> llvm::CodeGenOptLevel
{
    switch(level)
    {
    case OptimizationLevel::Debug:
...
[[nodiscard]] auto NativeTargetsAvailable() -> bool
{
...
[[nodiscard]] auto ArtifactObjectFormat(const llvm::Triple &triple) -> ObjectFormat
{
...
[[nodiscard]] auto CreateTargetMachine(llvm::Module &module, OptimizationLevel optimization,
                                       std::optional<Error> &error) -> std::unique_ptr<llvm::TargetMachine>
{
    ...
    auto machine = std::unique_ptr<llvm::TargetMachine>(target->createTargetMachine(
        triple, "generic", "", targetOptions, std::nullopt, std::nullopt, CodeGenerationLevel(optimization)));
    if(!machine)
    {
...
[[nodiscard]] auto EmitMachineCode(llvm::Module &module, llvm::TargetMachine &machine, MachineCodeEmission emission,
