...
module Visual.XSharp.Core.Optimizer
    ( CoreOptimizer (..), defaultCoreOptimizer, runCoreOptimizer ) where
...
foldPrimitive primitive values valueType = case (primitive, values) of
  ...
  where integer value = CoreLiteral (CoreInteger value) valueType; boolean value = CoreLiteral (CoreBoolean value) valueType
