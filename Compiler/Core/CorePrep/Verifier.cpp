...
namespace visual_xsharp::core
{
    ...
    {
        auto
        issue(std::string code, std::string message, SymbolId function, BlockId block = 0) -> VerificationIssue
        {
        ...
        void
        verify_instruction(const Instruction &instruction, SymbolId function, BlockId block, std::vector<VerificationIssue> &issues)
        {
        ...
        void
        verify_terminator(const Terminator &terminator, const std::unordered_set<BlockId> &block_ids, SymbolId function, BlockId block, std::vector<VerificationIssue> &issues)
        {
    ...
    verify(const CorePrepModule &module) -> std::vector<VerificationIssue>
    {
        ...
        issues.insert(issues.end(), std::make_move_iterator(semantic_issues.begin()), std::make_move_iterator(semantic_issues.end()));
        return issues;
    }
} // namespace visual_xsharp::core
