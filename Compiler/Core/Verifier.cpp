...
namespace Visual::XSharp::Core
{
    namespace
    {
        ...
        private:
            ...
            void
            CheckSymbol(const SymbolName &symbol, std::string code, std::string message)
            {
                if (symbol.id == 0U)
            ...
            AlwaysReturns(const std::vector<Statement> &statements) -> bool
            {
                for (const auto &statement : statements)
                {
                    if (statement.kind == Statement::Kind::Return)
                        return true;
                    if (statement.kind == Statement::Kind::If && !statement.falseBranch.empty() && AlwaysReturns(statement.trueBranch) && AlwaysReturns(statement.falseBranch))
                        return true;
                }
                return false;
            }
            ...
            void
            VerifyExpression(const Expression &expression, const Environment &environment)
            {
            ...
            VerifyCall(const Expression &expression, const Environment &environment)
            {
                ...
                for (std::size_t index = 0; index < comparable; ++index)
                    CheckSameType(calleeType.components[index], expression.operands[index].type, "VXC1023", "Core call argument has the wrong type");
                CheckSameType(calleeType.components.back(), expression.type, "VXC1024", "Core call result type disagrees with the callee");
            }
            void
            VerifyPrimitive(const Expression &expression, const Environment &environment)
            {
