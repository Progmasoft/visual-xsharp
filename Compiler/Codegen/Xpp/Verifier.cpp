...
[[nodiscard]] auto LiteralMatches(const IR::Operand &operand) -> bool
{
...
[[nodiscard]] auto ExpectedArity(IR::Opcode opcode) -> std::size_t
{
    switch(opcode)
    {
    case IR::Opcode::Copy:
...
[[nodiscard]] auto FunctionType(const IR::Function &function) -> Core::Type
{
...
void VerifyOperand(Context &context, const IR::Operand &operand,
...
void VerifyInstruction(Context &context, const IR::Instruction &value,
                       const std::unordered_map<IR::SymbolId, Core::Type> &storage,
                       const std::unordered_map<IR::SymbolId, const IR::Function *> &functions)
{
    if(value.opcode == IR::Opcode::Call)
    {
        if(value.operands.empty() || value.operands.front().kind != IR::Operand::Kind::Symbol ||
           value.operands.front().type.kind != Core::Type::Kind::Function)
...
void VerifyTerminator(Context &context, const IR::Terminator &value, const IR::Function &function,
                      const std::unordered_map<IR::SymbolId, Core::Type> &storage,
                      const std::unordered_map<IR::SymbolId, const IR::Function *> &functions,
                      const std::unordered_set<IR::BlockId> &blocks)
{
...
auto Verify(const ::visual_xsharp::xpp::Module &module) -> std::vector<VerificationIssue>
{
    Context context;
    ...
    for(const auto &function : module.functions)
        if(function.symbol.id == 0U || function.symbol.spelling.empty() ||
           !functions.emplace(function.symbol.id, &function).second)
        {
            ...
            for(std::size_t index = 0; index < block.instructions.size(); ++index)
            {
                ...
                if(instruction.effect == IR::Instruction::Effect::Define &&
                   (instruction.destination == 0U ||
                    !storage.emplace(instruction.destination, instruction.result_type).second))
