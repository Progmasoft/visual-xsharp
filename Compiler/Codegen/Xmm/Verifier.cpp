            ...
            else
            {
                for(std::size_t index = 1; index < instruction.operands.size(); ++index)
                    if(instruction.operands[index].type != signature[index - 1])
                        context.add(IssueKind::OperandType, "VXL1017",
                                    "call argument type does not match its signature");
    ...
    else
    {
                ...
                instruction.opcode == xmm::Opcode::NotBool)
        {
            if(instruction.result_type.kind != core::Type::Kind::Bool ||
               std::ranges::any_of(instruction.operands,
                                   [](const xmm::Value &value) { return value.type.kind != core::Type::Kind::Bool; }))
        ...
        else if(instruction.opcode >= xmm::Opcode::CompareLessI64 && instruction.opcode <= xmm::Opcode::CompareNotEqual)
        {
            if(instruction.result_type.kind != core::Type::Kind::Bool ||
               (instruction.operands.size() == 2 && instruction.operands[0].type != instruction.operands[1].type))
                context.add(IssueKind::OperandType, "VXL1023",
                            "comparison requires equal operand types and Bool result");
        }
        else if(instruction.opcode != xmm::Opcode::Call)
        {
            if(!IsInteger(instruction.result_type) ||
               std::ranges::any_of(instruction.operands, [&instruction](const xmm::Value &value)
                ...
                context.add(IssueKind::OperandType, "VXL1024",
                            "integer instruction operand and result types must agree");
        }
    }
...
auto Verify(const xmm::Module &module) -> std::vector<VerificationIssue>
{
    Context context;
    ...
    for(const auto &function : module.functions)
    {
        ...
        if(function.parameter_registers.size() != function.parameter_types.size())
            context.add(IssueKind::ParameterShape, "VXL1006",
        ...
        for(std::size_t index = 0; index < parameterCount; ++index)
        {
            if(function.parameter_registers[index] == 0 ||
               !registers.emplace(function.parameter_registers[index], function.parameter_types[index]).second)
                context.add(IssueKind::ParameterShape, "VXL1007",
                            "parameter virtual registers must be unique and non-zero");
