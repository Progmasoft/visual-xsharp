...
namespace xs::lil
{
    ...
    public:
        ...
        scalar(const TypeKind kind) noexcept
        {
            return Type{ { static_cast<XsLilTypeKind>(kind), 0U } };
        }

        [[nodiscard]] static constexpr Type
        aggregate(const std::uint32_t registry_id) noexcept
        {
            return Type{ { XS_LIL_TYPE_AGGREGATE, registry_id } };
        }

        [[nodiscard]] static constexpr Type
        array(const std::uint32_t registry_id) noexcept
        {
            return Type{ { XS_LIL_TYPE_ARRAY, registry_id } };
        }

        [[nodiscard]] constexpr TypeKind
        kind() const noexcept
        {
        ...
        registry_id() const noexcept
        {
            return value_.registry_id;
        }

        [[nodiscard]] constexpr XsLilType
        ...
        operator==(const Type &left, const Type &right) noexcept
        {
            return left.value_.kind == right.value_.kind && left.value_.registry_id == right.value_.registry_id;
        }

    private:
        explicit constexpr Type(const XsLilType value) noexcept
            : value_(value)
        {}
