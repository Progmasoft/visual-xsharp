...
XsBackendStatus xs_llvm_register_lil_types(XsLlvmCodegenUnit *unit, const XsLilModule *module, XsBackendError *error)
{
    ...
    for(size_t index = 0; index < count; ++index)
    {
        ...
        if(field_count != 0 && fields == nullptr)
            return aggregate_error(error, XS_BACKEND_SYSTEM_ERROR,
                                   "out of memory while lowering XLIL aggregate fields");
                                                    ...
                                                    XsBackendError *error)
{
    ...
    if(kind == XS_LIL_INSTRUCTION_ARRAY_GET || kind == XS_LIL_INSTRUCTION_ARRAY_SET)
    {
        ...
        LLVMBasicBlockRef trap_block =
            LLVMAppendBasicBlockInContext(unit->backend->context, llvm_function, "array.bounds");
    ...
    if(result_type.kind == XS_LIL_TYPE_ARRAY && unit->lil_array_dynamic[result_type.registry_id])
    {
        ...
        if(pointer == nullptr)
            return aggregate_error(error, XS_BACKEND_LLVM_ERROR,
                                   "LLVM could not allocate runtime-sized XLIL array storage");
        for(size_t field = 0; field < field_count; ++field)
        {
            XsLilValueId value = xs_lil_block_instruction_argument(block, index, field);
            if(value >= value_count || values[value] == nullptr)
                return aggregate_error(error, XS_BACKEND_INVALID_ARGUMENT,
                                       "XLIL array references an unavailable element");
            LLVMValueRef offset = LLVMConstInt(i64, field, false);
            LLVMValueRef element = LLVMBuildGEP2(builder, unit->lil_array_elements[result_type.registry_id], pointer,
                                                 &offset, 1, "array.element");
        ...
        values[result] =
            LLVMBuildInsertValue(builder, view, LLVMConstInt(i64, field_count, false), 1, "array.with.length");
        return values[result] == nullptr
    ...
    for(size_t field = 0; field < field_count; ++field)
    {
        ...
        if(value >= value_count || values[value] == nullptr)
            return aggregate_error(error, XS_BACKEND_INVALID_ARGUMENT,
                                   "XLIL aggregate references an unavailable field");
