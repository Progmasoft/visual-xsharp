...
namespace xs::lil
{
    ...
    Block
    Builder::append_block(const Function function, const std::string_view label)
    {
    ...
    void
    Builder::position_at_end(const Block block)
    {
        XsLilError error{};
        throw_if_failed(xs_lil_builder_position_at_end(value_, block.native_handle(), &error), error, "position XLIL builder");
    }

    ValueId
    Builder::constant_i32(const std::int32_t value)
    {
    ...
    ValueId
    Builder::constant_i64(const std::int64_t value)
    {
        ValueId result{ XS_LIL_INVALID_VALUE_ID };
        XsLilError error{};
    ...
    ValueId
    Builder::constant_bool(const bool value)
    {
        ValueId result{ XS_LIL_INVALID_VALUE_ID };
        XsLilError error{};
    ...
    ValueId
    Builder::call(const std::string_view callee, const std::span<const ValueId> arguments)
    {
        const std::string owned_callee{ callee };
        ValueId result{ XS_LIL_INVALID_VALUE_ID };
        XsLilError error{};
        throw_if_failed(
            xs_lil_builder_call(value_, owned_callee.c_str(), arguments.data(), arguments.size(), &result, &error),
            error,
            "build XLIL call");
        return result;
    }
    ...
    void
    Builder::return_value(const ValueId value)
    {
        XsLilError error{};
    ...
    void
    Builder::branch(const Block target)
    {
        XsLilError error{};
    ...
    void
    Builder::branch_if(const ValueId condition, const Block then_block, const Block else_block)
    {
        XsLilError error{};
        throw_if_failed(
            xs_lil_builder_branch_if(value_, condition, then_block.native_handle(), else_block.native_handle(), &error),
            error,
            "build XLIL conditional branch");
    }

    XsLilBuilder *
    Builder::native_handle() noexcept
    {
        return value_;
    }
} // namespace xs::lil
