...
namespace xs::lil
{
    Error::Error(const XsLilStatus status, std::string message)
    ...
    void
    throw_if_failed(const XsLilStatus status, const XsLilError &error, const std::string_view operation)
    {
        if (status == XS_LIL_OK)
            return;
