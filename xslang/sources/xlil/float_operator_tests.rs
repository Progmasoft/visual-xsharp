...
fn roundtrips_all_float_operations_for_both_widths()
{
    ...
    {
        let mut function = Function::definition(format!("ops_{}", super::type_name(value_type)), value_type, vec![
        ...
        {
            result = function
                .binary_float(block, operation, value_type, result, right)
...
fn roundtrips_xmir_and_lowers_float_operations()
{
    ...
    assert!(matches!(lowered.blocks[0].instructions[2], Instruction::BinaryFloat {
        operation: FloatBinaryOperation::Add,
        value_type: Type::F64,
        ..
    }));
...
fn rejects_mismatched_float_operation_types()
{
    ...
    assert!(
        function
            .binary_float(block, FloatBinaryOperation::Add, Type::F32, left, right)
            .is_none()
    );
}
