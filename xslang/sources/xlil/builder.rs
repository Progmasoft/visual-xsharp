  ...
  fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result
  {
    match self
    {
      ...
      Self::ArgumentCount { function,
                            expected,
                            actual, } => write!(formatter,
                                                "XLIL call to '{function}' expects {expected} argument(s), got \
                                                 {actual}"),
      Self::ArgumentType { function,
                           index,
                           expected,
                           actual, } => write!(formatter,
                                               "XLIL call to '{function}' argument {index} expects {expected:?}, got \
                                                {actual:?}"),
      Self::UnexpectedCallResult { function,
                                   return_type, } => write!(formatter,
                                                            "XLIL call to '{function}' has incompatible result type \
                                                             {return_type:?}"),
      ...
      Self::Verification(diagnostics) => write!(formatter,
...
impl Builder
{
                      ...
                      -> Result<Option<ValueId>, BuildError>
  {
    ...
    for (index, (argument, expected)) in arguments.iter().zip(parameters).enumerate()
    {
      ...
      {
        return Err(BuildError::ArgumentType { function,
                                              index,
                                              expected,
                                              actual });
      }
  ...
  pub fn call_value(&mut self, function: impl Into<String>, arguments: Vec<ValueId>) -> Result<ValueId, BuildError>
  {
    ...
    self.call_checked(function.clone(), arguments)?
        .ok_or(BuildError::UnexpectedCallResult { function,
  ...
  {
    let function = function.into();
    let return_type = self.module
                          .function(&function)
                          .map(|callee| callee.return_type)
                          .ok_or_else(|| BuildError::UnknownFunction(function.clone()))?;
    if return_type != Type::VOID
    {
      return Err(BuildError::UnexpectedCallResult { function,
                                                    return_type });
    }
  ...
  pub fn branch_if(&mut self, condition: ValueId, then_block: BlockId, else_block: BlockId) -> Result<(), BuildError>
  {
    let block = self.block()?;
    self.function_mut()?
