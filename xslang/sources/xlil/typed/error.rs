...
impl fmt::Display for TypedBuildError
{
  fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result
  {
    match self
    {
      ...
      Self::ParameterOutOfRange { index,
                                  count, } =>
      {
        write!(formatter,
      ...
      Self::MissingCallResult { function } => write!(formatter, "XLIL call to '{function}' returns no value"),
      Self::UnexpectedCallResult { function,
                                   actual, } =>
      {
        write!(formatter, "XLIL void call to '{function}' returns {actual:?}")
      }
      Self::IntegerConstantOutOfRange { value_type,
                                        bits, } =>
      {
        write!(formatter,
