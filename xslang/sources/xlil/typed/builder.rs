...
impl TypedBuilder
{
  ...
  {
    Self { raw: Builder::new(module_name),
  ...
  pub fn parameter<T: XlilType>(&self, index: usize) -> Result<Value<T>, TypedBuildError>
  {
    ...
    {
      return Err(TypedBuildError::ParameterOutOfRange { index,
  ...
  {
    let constant =
      IntegerConstant::new(T::XLIL_TYPE, bits).ok_or(TypedBuildError::IntegerConstantOutOfRange { value_type:
                                                                                                    T::XLIL_TYPE,
                                                                                                  bits })?;
  ...
  pub fn const_utf32_units(&mut self,
  ...
  pub fn integer<T: IntegerType>(&mut self,
                                 ...
                                 -> Result<Value<T>, TypedBuildError>
  {
    ...
    {
      return Err(TypedBuildError::TypeMismatch { operation: "integer arithmetic result",
  ...
  pub fn compare_integer<T: IntegerType>(&mut self,
                                         operation: IntegerBinaryOperation,
                                         left: Value<T>,
                                         right: Value<T>)
                                         -> Result<Value<bool>, TypedBuildError>
  {
    if !operation.is_comparison()
    {
      return Err(TypedBuildError::TypeMismatch { operation: "integer comparison result",
                                                 expected: Type::BOOL,
                                                 actual: T::XLIL_TYPE });
    }
    let id = self.raw
  ...
  pub fn float<T: FloatType>(&mut self,
  ...
  pub fn compare_float<T: FloatType>(&mut self,
                       ...
                       -> Result<Value<bool>, TypedBuildError>
  {
    ...
    Ok(Value::trusted(self.raw.compare_str(operation, left.id(), right.id())?))
  }
  ...
  pub fn call<T: XlilType>(&mut self,
                           ...
                           -> Result<Value<T>, TypedBuildError>
  {
    ...
    {
      return Err(TypedBuildError::MissingCallResult { function });
    };
    Ok(Value::trusted(result))
  }

  /// Emits a checked call to a void function.
  pub fn call_void(&mut self,
                   function: impl Into<String>,
                   arguments: impl IntoIterator<Item = AnyValue>)
                   -> Result<(), TypedBuildError>
  {
    ...
    if actual != Type::VOID
    {
      return Err(TypedBuildError::UnexpectedCallResult { function,
                                                         actual });
    }
    self.raw
...
fn ensure_type(operation: &'static str, expected: Type, actual: Type) -> Result<(), TypedBuildError>
{
  ...
  {
    Err(TypedBuildError::TypeMismatch { operation,
                                        expected,
                                        actual })
  }
}
...
fn ensure_arithmetic_float(value_type: Type) -> Result<(), TypedBuildError>
{
  ...
  {
    Err(TypedBuildError::TypeMismatch { operation: "floating arithmetic",
                                        expected: Type::F32,
                                        actual: value_type })
  }
}
