...
use crate::xlil::{
    FloatBinaryOperation, FloatComparisonOperation, IntegerBinaryOperation, StrComparisonOperation, Type,
    Utf32Encoding, module_to_string, parse_verified,
...
fn typed_integer_function_roundtrips_through_canonical_text()
{
    ...
    builder
        .begin(Signature::returning::<i64>("sum").parameter::<i64>().parameter::<i64>())
        .unwrap();
    builder.append_block("entry").unwrap();
...
fn exact_width_integer_constants_reject_excess_bits_early()
{
    ...
    assert_eq!(error, TypedBuildError::IntegerConstantOutOfRange {
        value_type: Type::I8,
        bits: 0x1ff
    });
}
...
fn arithmetic_method_rejects_comparison_operations()
{
    ...
    assert_eq!(error, TypedBuildError::TypeMismatch {
...
fn comparison_method_rejects_arithmetic_operations()
{
    ...
    let right = builder.const_i32(2).unwrap();
    let error = builder
        .compare_integer(IntegerBinaryOperation::Add, left, right)
        .unwrap_err();
    assert_eq!(error, TypedBuildError::TypeMismatch {
...
fn typed_branch_requires_boolean_by_construction()
{
    ...
    builder
        .begin(
            Signature::returning::<i32>("choose")
                .parameter::<i32>()
                .parameter::<i32>(),
        )
        .unwrap();
    ...
    let condition = builder
        .compare_integer(IntegerBinaryOperation::Greater, left, right)
        .unwrap();
...
fn boolean_negation_produces_another_boolean()
{
    ...
    builder
        .begin(Signature::returning::<bool>("invert").parameter::<bool>())
        .unwrap();
    builder.append_block("entry").unwrap();
...
fn typed_float_comparison_returns_bool()
{
    ...
    let result = builder
        .compare_float(FloatComparisonOperation::Less, left, right)
        .unwrap();
    builder.return_value(result).unwrap();
    let text = module_to_string(&builder.finish().unwrap());
...
fn non_native_float_arithmetic_stays_explicitly_deferred()
{
    ...
    builder
        .begin(Signature::returning::<F16>("half").parameter::<F16>())
        .unwrap();
    builder.append_block("entry").unwrap();
...
fn typed_utf32_comparison_hides_rust_source_text()
{
    ...
    let result = builder
        .compare_utf32(StrComparisonOperation::Equal, left, right)
        .unwrap();
    builder.return_value(result).unwrap();
...
fn utf32_comparison_rejects_erased_non_string_values()
{
    ...
    let error = builder
        .compare_utf32(StrComparisonOperation::Equal, integer, text)
        .unwrap_err();
    assert_eq!(error, TypedBuildError::TypeMismatch {
        operation: "UTF-32 comparison",
        expected: Type::STR,
        actual: Type::I32
    });
}
...
fn typed_checked_calls_validate_arguments_and_result()
{
    ...
    builder
        .declare(Signature::returning::<i32>("identity").parameter::<i32>())
        .unwrap();
...
fn typed_void_call_rejects_value_producing_callee()
{
    ...
    assert_eq!(error, TypedBuildError::UnexpectedCallResult {
...
fn parameter_lookup_rejects_wrong_marker_and_index()
{
    ...
    builder
        .begin(Signature::returning::<i32>("main").parameter::<i32>())
        .unwrap();
    builder.append_block("entry").unwrap();
    assert_eq!(
        builder.parameter::<i64>(0).unwrap_err(),
        TypedBuildError::TypeMismatch {
            operation: "parameter",
            expected: Type::I64,
            actual: Type::I32
        }
    ...
    assert_eq!(
        builder.parameter::<i32>(1).unwrap_err(),
        TypedBuildError::ParameterOutOfRange {
            index: 1,
            count: 1
        }
    );
...
fn return_contract_is_checked_before_raw_verification()
{
    ...
    assert_eq!(
        builder.return_value(narrow).unwrap_err(),
        TypedBuildError::TypeMismatch {
            operation: "return",
            expected: Type::I32,
            actual: Type::I64
...
fn void_return_contract_rejects_value_function()
{
    ...
    assert_eq!(builder.return_void().unwrap_err(), TypedBuildError::TypeMismatch {
        operation: "void return",
        expected: Type::VOID,
        actual: Type::I32
    });
}
