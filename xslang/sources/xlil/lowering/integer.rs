...
impl MirToXlilLowerer
{
    ...
    #[allow(clippy::too_many_arguments)]
    pub(super) fn lower_const_integer(
        &mut self,
        local: mir::LocalId,
    ...
    )
    {
        ...
        match local_types.get(&local).copied().flatten()
        {
            ...
            {
                let constant =
                    IntegerConstant::new(expected, value.bits()).expect("verified MIR integer fits its type");
    ...
    #[allow(clippy::too_many_arguments)]
    pub(super) fn lower_binary_integer(
        &mut self,
        result: mir::LocalId,
        left: mir::LocalId,
        right: mir::LocalId,
