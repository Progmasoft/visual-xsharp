...
pub fn type_ref_to_checked(value: &TypeRef) -> Option<type_check::Type>
{
    Some(match value
    {
        ...
        TypeRef::Optional {
            ...
            element: Box::new(type_ref_to_checked(element)?),
        },
        TypeRef::Result {
            success,
            error,
        } => type_check::Type::Result {
            success: Box::new(type_ref_to_checked(success)?),
            error: Box::new(type_ref_to_checked(error)?),
        },
        TypeRef::Reference {
            referent,
            mutable,
        } => type_check::Type::Reference {
            referent: Box::new(type_ref_to_checked(referent)?),
            mutable: *mutable,
        },
        TypeRef::Array {
            element,
        ...
        TypeRef::Map {
            ...
            key: Box::new(type_ref_to_checked(key)?),
        ...
        TypeRef::Tuple {
            ...
            fields: fields
                .iter()
                .map(|field| {
                    Some(type_check::TupleFieldType {
                        name: field.name.clone(),
                        ty: type_ref_to_checked(&field.ty)?,
                    })
                })
...
pub(crate) fn validate_layouts(declarations: &[NominalType]) -> Vec<(String, LayoutError)>
{
    ...
    declarations
        ...
        .filter_map(|declaration| {
            resolved_fields(declaration, &definitions)
                .err()
