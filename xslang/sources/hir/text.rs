...
pub use program::{
    XhirProgram, parse_xhir_program, program_to_xhir, program_to_xhir_with_declarations,
    program_to_xhir_with_parameters,
};
...
pub fn parse_xhir_header(text: &str) -> Option<XhirDocumentHeader>
{
    ...
    {
        return Some(XhirDocumentHeader {
            version,
...
pub fn parse_xhir_typecheck_analysis(text: &str) -> Result<Vec<TypeDiagnostic>, Vec<XhirParseDiagnostic>>
{
    ...
    else
    {
        ...
        return Err(vec![XhirParseDiagnostic {
            line: 1,
            message,
        }]);
    };
    ...
    while let Some((line_index, line)) = lines.next()
    {
        ...
        else
        {
            ...
            diagnostics.push(XhirParseDiagnostic {
                line: line_index + 1,
                message,
            });
            continue;
...
include!("text/expression_writer.rs");
