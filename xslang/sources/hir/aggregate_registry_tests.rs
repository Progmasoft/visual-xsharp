...
fn enum_data(name: &str, bases: &[&str], variants: Vec<EnumVariant>) -> NominalType
{
  NominalType { name: name.to_string(),
                ...
                bases: bases.iter()
                            .map(|name| Base { ty: TypeRef::Named((*name).to_string()),
...
{
  let declaration = enum_data("Value", &[], vec![variant("Number", Some(PrimitiveType::Int), 0),
...
{
  let declaration = enum_data("Value", &[], vec![variant("Number", Some(PrimitiveType::Int), 17),
  ...
  assert_eq!((&int.owner, &int.name, int.tag, int.payload_type),
             (&"Value".to_string(), &"Number".to_string(), 0, Some(Type::I64)));
  assert_eq!((&long.owner, &long.name, long.tag, long.payload_type),
...
fn inherited_enum_data_layout_flattens_base_variants_before_derived_ones()
{
  ...
  let leaf = enum_data("Leaf", &["Root"], vec![variant("LeafValue",
  ...
  assert_eq!(leaf.variants
                 ...
                 .collect::<Vec<_>>(),
             ["Root", "Leaf", "Leaf"]);
  assert_eq!(leaf.variants.iter().map(|variant| variant.tag).collect::<Vec<_>>(), [0,
                                                                                   1,
                                                                                   2]);
...
{
  let regular = NominalType { name: "Color".to_string(),
...
{
  let invalid = enum_data("Value", &[], vec![variant("Number", Some(PrimitiveType::Long), 0),
                                             variant("Number", Some(PrimitiveType::Long), 1)]);
...
fn lookup_requires_complete_variant_identity()
{
  ...
  assert_eq!(lookup("Value").and_then(|layout| layout.variant("Value", "Number", 0))
                            .and_then(|variant| variant.payload_type),
             Some(Type::I64));
  assert_eq!(lookup("Value").and_then(|layout| layout.variant("Value", "Number", 1))
                            .and_then(|variant| variant.payload_type),
             Some(Type::I32));
...
{
  let first = enum_data("First", &[], vec![variant("A", Some(PrimitiveType::Long), 99),
                                           variant("B", None, 100)]);
  let second = enum_data("Second", &[], vec![variant("C", Some(PrimitiveType::Int), 44),
                                             variant("D", Some(PrimitiveType::Long), 45)]);
  let registry = aggregate_registry::build(&[first, second]).unwrap();
  assert_eq!(registry.enum_data
                     .iter()
...
fn enum_data_registry_ids_follow_preceding_data_and_enum_layouts()
{
  ...
  let point = NominalType { name: "Point".to_string(),
                            ...
                            variants: Vec::new(),
                            span: span() };
  let color = NominalType { name: "Color".to_string(),
                            kind: NominalKind::Enum,
                            bases: Vec::new(),
                            fields: Vec::new(),
                            variants: vec![variant("Red", None, 0)],
