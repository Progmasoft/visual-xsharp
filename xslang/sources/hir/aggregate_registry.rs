...
) -> Option<()>
{
    ...
    match expression
    {
        ...
        Expression::Unary {
            operand, ..
        } => visit_expression(operand, definitions, registry)?,
...
) -> Option<Type>
{
    match value
    {
        ...
        HirType::Reference {
            referent,
            mutable: false,
        } if **referent == HirType::Primitive(crate::hir::type_check::PrimitiveType::Str) => Some(Type::STR),
        HirType::Optional {
            element,
        } =>
        {
            ...
            registry.layouts.push(AggregateLayout {
                name: format!("optional.{}", registry.optionals.len()),
                value_type,
        ...
        } =>
        {
            ...
            let value_type = Type::aggregate(registry.layouts.len() as u32);
            registry.layouts.push(AggregateLayout {
                name: format!("result.{}", registry.results.len()),
                value_type,
        ...
        } =>
        {
            ...
            let value_type = Type::aggregate(registry.layouts.len() as u32);
            registry.layouts.push(AggregateLayout {
                name: format!("tuple.{}", registry.tuples.len()),
                value_type,
                fields: field_types,
            });
...
{
    let owned_definitions = declarations
        .iter()
        .cloned()
        ...
        .filter(|declaration| declaration.kind == NominalKind::Enum)
    {
        if !declaration.bases.is_empty() ||
            ...
            registry.types.contains_key(&declaration.name)
        {
            return None;
        }
        let value_type = Type::aggregate(registry.layouts.len() as u32);
        ...
        .filter(|declaration| declaration.kind == NominalKind::EnumData)
    {
        ...
        for variant in enum_data.variants(&declaration.name).ok()?
        {
            ...
            variants.push(EnumDataVariantLayout {
        ...
        registry.enum_data.push(EnumDataLayout {
            name: declaration.name.clone(),
            value_type,
            variants,
        });
