    ...
    Function {
        ...
        body: vec![Statement::Return {
            value: Some(Expression::Match {
                ...
                selector_type: Box::new(Type::Named(enum_type.to_string())),
                arms,
...
fn accepts_exhaustive_enum_data_statement_without_else()
{
    ...
    let function = statement("Token", vec![
        arm(
            enum_pattern("Token", "Token", "Text", 0, Some("text"), Some(PrimitiveType::String)),
            None,
        ),
        arm(
            enum_pattern("Token", "Token", "Integer", 1, Some("number"), Some(PrimitiveType::Int)),
            None,
        ),
        arm(enum_pattern("Token", "Token", "End", 2, None, None), None),
    ]);
    let diagnostics = TypeChecker::new()
        .with_nominal_types(&[token])
...
fn requires_else_when_one_enum_data_variant_is_missing()
{
    ...
    let function = statement("Token", vec![
        arm(
            enum_pattern("Token", "Token", "Text", 0, None, Some(PrimitiveType::String)),
            None,
        ),
        arm(enum_pattern("Token", "Token", "End", 2, None, None), None),
    ]);
...
{
    let value = declaration("Value", &[], vec![
        variant("Number", Some(PrimitiveType::Int), 0),
        variant("Number", Some(PrimitiveType::Long), 1),
    ]);
    let function = statement("Value", vec![
        arm(
            enum_pattern("Value", "Value", "Number", 0, Some("number"), Some(PrimitiveType::Bool)),
            None,
        ),
        arm(MatchPattern::Else, None),
    ]);
    let diagnostics = TypeChecker::new()
        .with_nominal_types(&[value])
        .check_function(&function);
    assert!(
        diagnostics
            .iter()
...
fn rejects_forged_owner_or_flattened_tag()
{
    ...
    let function = statement("Token", vec![
        arm(
            enum_pattern("Token", "Base", "Text", 99, None, Some(PrimitiveType::String)),
            None,
        ),
        arm(MatchPattern::Else, None),
    ]);
...
fn payload_binding_is_visible_inside_its_arm()
{
    ...
    let function = expression("Value", vec![
        arm(
            enum_pattern("Value", "Value", "Number", 0, Some("number"), Some(PrimitiveType::Long)),
            Some(local("number")),
        ),
        arm(
            enum_pattern("Value", "Value", "Empty", 1, None, None),
...
fn inherited_variants_participate_in_exhaustiveness()
{
    ...
    let child = declaration("Value", &["Root"], vec![variant(
        "Text",
    ...
    let function = statement("Value", vec![
        arm(
            enum_pattern("Value", "Root", "Number", 0, None, Some(PrimitiveType::Long)),
            None,
        ),
        arm(
            enum_pattern("Value", "Value", "Text", 1, None, Some(PrimitiveType::String)),
            None,
        ),
    ]);
    let diagnostics = TypeChecker::new()
        .with_nominal_types(&[root, child])
        .check_function(&function);
    assert!(diagnostics.is_empty(), "{diagnostics:#?}");
}
...
fn inherited_match_rejects_declaration_tag_in_place_of_flattened_tag()
{
    ...
    let function = statement("Value", vec![
        arm(
            enum_pattern("Value", "Root", "Number", 7, None, Some(PrimitiveType::Long)),
            None,
        ),
        arm(MatchPattern::Else, None),
    ]);
