...
fn value_declaration() -> NominalType
{
  NominalType { name: "Value".to_string(),
                ...
                variants: vec![EnumVariant { name: "Number".to_string(),
                                             ...
                                             tag: 0,
                                             span: source_span() },
                               EnumVariant { name: "Number".to_string(),
                                             payload: Some(TypeRef::Primitive(PrimitiveType::Long)),
                                             tag: 1,
                                             span: source_span() },
                               EnumVariant { name: "Empty".to_string(),
                                             payload: None,
                                             tag: 2,
                                             span: source_span() }],
                span: source_span() }
...
{
  Expression::EnumData { enum_type: "Value".to_string(),
                         owner: "Value".to_string(),
                         variant: "Number".to_string(),
                         tag,
                         payload: Some(Box::new(Expression::Local { name: "payload".to_string(),
                                                                    span: span() })),
...
{
  Function { name: "wrap".to_string(),
             return_type: Some(Type::Named("Value".to_string())),
             locals: vec![Local { name: "payload".to_string(),
...
fn accepts_exact_enum_data_payload_metadata()
{
  ...
  assert!(TypeChecker::new().with_nominal_types(&[declaration])
...
fn rejects_spoofed_enum_data_tag()
{
  ...
  let diagnostics = TypeChecker::new().with_nominal_types(&[declaration])
                                      .check_function(&checked);
...
fn rejects_payload_type_that_does_not_match_selected_overload()
{
  ...
  let Expression::EnumData { payload_type, .. } = &mut value
  else
  {
    unreachable!();
  };
...
{
  let declaration = value_declaration();
  let value = Expression::EnumData { enum_type: "Value".to_string(),
                                     owner: "Value".to_string(),
                                     ...
                                     payload_type: None,
                                     span: span() };
  let checked = Function { name: "empty".to_string(),
                           return_type: Some(Type::Named("Value".to_string())),
                           locals: Vec::new(),
                           body: vec![Statement::Return { value: Some(value),
                                                          span: span() }] };
