...
{
  Expression::Call { function: name.to_string(),
                     arguments: vec![value],
...
{
  Expression::ResultPropagation { value: Box::new(value),
                                  span: span() }
}

fn binding(name: &str, ty: Type, initializer: Expression) -> Statement
{
  Statement::Let { local: Local { name: name.to_string(),
                                  ty,
                                  mutable: false,
                                  span: span() },
...
{
  Function { name: "Forward".to_string(),
             return_type: Some(result_long()),
             locals: vec![],
             body: vec![binding("input", result_long(), ok("7")),
                        binding("value", long(), propagation(local("input"))),
                        Statement::Return { value: Some(constructor("Ok", local("value"), long(), result_long())),
                                            span: span() },] }
}

fn failure_function() -> Function
{
  Function { name: "ForwardFailure".to_string(),
             return_type: Some(result_long()),
             locals: vec![],
             body: vec![binding("input", result_long(), error("13")),
                        binding("value", long(), propagation(local("input"))),
                        Statement::Return { value: Some(constructor("Ok", local("value"), long(), result_long())),
                                            span: span() },] }
}
...
fn lower(function: &Function) -> (mir::Function, crate::xlil::Module)
{
  ...
  let registry =
    aggregate_registry::build_functions_with_nominals(&[], std::slice::from_ref(function)).expect("Result registry \
                                                                                                   should build");
...
fn xhir_round_trips_structured_result_and_propagation()
{
  ...
  assert!(matches!(&parsed.body[1], Statement::Let { initializer:
...
fn lowers_ok_and_propagation_to_explicit_mir_control_flow()
{
  ...
  assert!(mir.blocks
             ...
             .any(|statement| matches!(statement, mir::Statement::Aggregate { field_types, .. }
  ...
  let parsed = crate::xlil::parse_module(&xlil).unwrap_or_else(|diagnostics| {
                                                 panic!("Result XLIL should parse:\n{xlil}\n{diagnostics:#?}")
                                               });
  ...
  let failure = mir.blocks
                   ...
                   .find(|block| {
                     ...
                     block.statements.iter().any(|statement| {
                                              matches!(statement, mir::Statement::Extract { field: 2,
                                                                                            .. })
                                            })
                   })
...
fn result_registry_deduplicates_identical_layouts()
{
  ...
  assert_eq!(registry.layouts[0].fields, vec![XlilType::BOOL,
                                              XlilType::I32,
                                              XlilType::I32]);
}
...
fn result_registry_supports_nested_payloads()
{
  ...
  let function = Function { name: "Nested".to_string(),
  ...
  let outer_type = registry.results
                           .iter()
...
fn type_checker_rejects_propagation_outside_result_function()
{
  let invalid = Function { name: "Invalid".to_string(),
                           ...
                           locals: vec![],
                           body: vec![binding("input", result_long(), ok("7")),
                                      Statement::Return { value: Some(propagation(local("input"))),
                                                          span: span() },] };
...
fn type_checker_rejects_mismatched_propagation_error_type()
{
  ...
  let invalid = Function { name: "InvalidError".to_string(),
                           return_type: Some(result_long()),
                           locals: vec![],
                           body: vec![binding("input", input_type, input),
                                      binding("value", long(), propagation(local("input"))),
                                      Statement::Return { value: Some(ok("1")),
                                                          span: span() },] };
  let diagnostics = crate::hir::type_check::TypeChecker::new().check_function(&invalid);
  assert!(diagnostics.iter()
                     .any(|diagnostic| { diagnostic.code == TypeDiagnosticCode::ResultPropagationReturnMismatch }));
...
{
  let invalid = Function { name: "InvalidConstructor".to_string(),
                           return_type: Some(result_long()),
                           locals: vec![],
...
fn result_match_function(initializer: Expression) -> Function
{
  Function { name: "Read".to_string(),
             return_type: Some(long()),
             locals: vec![],
             body: vec![binding("input", result_long(), initializer),
                        Statement::Return { value: Some(Expression::Match { selector:
                                                                              Box::new(local("input")),
                                                                            selector_type:
                                                                              Box::new(result_long()),
                                                                            arms: vec![result_arm(true,
                                                                                                  "value"),
                                                                                       result_arm(false,
                                                                                                  "failure")],
...
fn type_checker_rejects_duplicate_result_match_variants()
{
  ...
  let Statement::Return { value: Some(Expression::Match { arms, .. }),
                          .. } = &mut invalid.body[1]
  else
  {
...
{
  let invalid = Function { name: "InvalidMatch".to_string(),
                           return_type: Some(long()),
                           locals: vec![],
                           body: vec![Statement::Return { value:
                                                            Some(Expression::Match { selector:
                                                                                       Box::new(integer("7")),
                                                                                     selector_type: Box::new(long()),
                                                                                     arms:
                                                                                       vec![result_arm(true, "value"),
  ...
  assert!(diagnostics.iter().any(|diagnostic| {
                              diagnostic.code == TypeDiagnosticCode::LiteralTypeMismatch &&
...
fn type_checker_rejects_result_pattern_with_inconsistent_payload_type()
{
  ...
  let MatchPattern::ResultVariant { payload_type, .. } = &mut arms[0].pattern
  else
  {
