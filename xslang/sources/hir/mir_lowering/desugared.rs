...
impl HirToMirLowerer
{
    pub(super) fn surface_statement_from_desugared(&mut self, statement: &DesugaredStatement) -> Option<Statement>
    {
        match statement
        {
            ...
            {
                self.surface_expression_from_desugared(expression).map(Statement::Expr)
            }
            ...
            DesugaredStatement::If {
                ...
                condition: self.surface_expression_from_desugared(condition)?,
            ...
            } =>
            {
                ...
                Some(Statement::Match {
                    selector: self.surface_expression_from_desugared(selector)?,
    ...
    pub(super) fn surface_expression_from_desugared(&mut self, expression: &DesugaredExpression) -> Option<Expression>
    {
        match expression
        {
            ...
            DesugaredExpression::Member {
                ...
                receiver: Box::new(self.surface_expression_from_desugared(receiver)?),
                owner: owner.clone(),
                name: name.clone(),
            ...
            DesugaredExpression::TupleElement {
                ...
                tuple: Box::new(self.surface_expression_from_desugared(tuple)?),
                index: *index,
                element_type: element_type.clone(),
                span: *span,
            }),
            DesugaredExpression::Index {
                ...
                collection: Box::new(self.surface_expression_from_desugared(collection)?),
            ...
            DesugaredExpression::Assign {
                ...
                .surface_expression_from_desugared(value)
                .map(|value| Expression::Assign {
                    target: target.clone(),
                    value: Box::new(value),
                    span: *span,
                }),
            DesugaredExpression::AssignField {
                ...
                .surface_expression_from_desugared(value)
                .map(|value| Expression::AssignField {
                    target: target.clone(),
                    value: Box::new(value),
                    span: *span,
                }),
            ...
            DesugaredExpression::If {
                ...
                condition: Box::new(self.surface_expression_from_desugared(condition)?),
            ...
            DesugaredExpression::Match {
                ...
                selector: Box::new(self.surface_expression_from_desugared(selector)?),
                selector_type: selector_type.clone(),
                arms: arms
                    .iter()
                    .map(|arm| {
