...
{
  Type::Optional { element: Box::new(ty) }
}
...
{
  Expression::OptionalMember { receiver: Box::new(receiver),
...
fn function(initializer: Expression, member: &str, fallback: &str) -> Function
{
  Function { name: "main".to_string(),
             ...
             locals: vec![],
             body: vec![
      Statement::Let { local: Local { name: "point".to_string(),
      ...
      Statement::Return {
        value: Some(Expression::Binary {
          ...
          left: Box::new(optional_member(Expression::Local { name: "point".to_string(),
                                                             span: span() },
                                         member)),
...
fn lower(function: &Function) -> (mir::Function, crate::xlil::Module)
{
  ...
  let registry =
    crate::hir::aggregate_registry::build_functions_with_nominals(std::slice::from_ref(&declaration),
...
fn xhir_round_trips_optional_member_access()
{
  ...
  let Statement::Return { value: Some(Expression::Binary { left, .. }),
                          .. } = &parsed.body[1]
  else
  {
...
fn lowers_present_optional_member_through_mir_and_xlil()
{
  ...
  assert!(mir.blocks
             ...
             .count() >=
          2);
  let extracted_fields = mir.blocks
                            .iter()
                            .flat_map(|block| &block.statements)
                            .filter_map(|statement| match statement
                            {
                              mir::Statement::Extract { field, .. } => Some(*field),
                              _ => None,
                            })
                            .collect::<Vec<_>>();
...
{
  let declaration = point_declaration();
  let invalid = Function { name: "bad".to_string(),
                           return_type: Some(optional(long())),
                           locals: vec![],
                           body: vec![Statement::Return { value: Some(optional_member(point("1", "2"), "x")),
                                                          span: span() }] };
  let diagnostics = crate::hir::type_check::TypeChecker::new().with_nominal_types(std::slice::from_ref(&declaration))
...
fn type_checker_rejects_an_unknown_optional_field()
{
  ...
  let diagnostics = crate::hir::type_check::TypeChecker::new().with_nominal_types(std::slice::from_ref(&point))
                                                              .check_function(&invalid);
  assert!(diagnostics.iter().any(|diagnostic| {
                              diagnostic.code == crate::hir::type_check::DiagnosticCode::UnknownField &&
...
fn type_checker_rejects_a_mismatched_optional_member_result()
{
  let point = point_declaration();
  let invalid_member =
    Expression::OptionalMember { receiver: Box::new(Expression::Local { name: "point".to_string(),
                                                                        span: span() }),
                                 owner: "Point".to_string(),
                                 name: "x".to_string(),
                                 field_type: Box::new(long()),
  ...
  let invalid = Function { name: "bad".to_string(),
                           ...
                           body: vec![Statement::Let { local: Local { name: "point".to_string(),
                                                       ...
                                                       initializer: Some(Expression::Literal { literal:
                                                                                                 Literal::None,
                                                                                               span: span() }) },
                                      Statement::Return { value: Some(invalid_member),
                                                          span: span() },] };
  let diagnostics = crate::hir::type_check::TypeChecker::new().with_nominal_types(std::slice::from_ref(&point))
                                                              .check_function(&invalid);
  assert!(diagnostics.iter().any(|diagnostic| {
