...
fn lowers_str_literal_through_target_independent_mir()
{
    let function = Function {
        ...
        return_type: Some(Type::Reference {
            referent: Box::new(primitive(PrimitiveType::Str)),
            mutable: false,
        }),
        locals: vec![],
        body: vec![Statement::Return {
    ...
    let xlil = crate::xlil::lowering::MirToXlilLowerer::new()
        .with_utf32_encoding(crate::xlil::Utf32Encoding::BigEndian)
        .lower_function(&mir)
...
fn lowers_char_literal_as_one_u32_scalar()
{
    ...
    assert!(matches!(mir.blocks[0].statements[0], mir::Statement::ConstInteger {
        value: mir::IntegerConstant::U32(0x000003a9),
        ..
    }));
    assert!(verify_function(&mir).is_empty());
    let xmir = crate::mir::text::function_to_xmir(&mir);
...
{
    let parameter = super::super::type_check::Local {
    ...
    let function = Function {
        ...
        body: vec![Statement::Return {
            value: Some(Expression::Call {
                ...
                arguments: vec![
                    Expression::Literal {
                        literal: Literal::Integer("3".to_string()),
                        span: span(10, 11),
                    },
                    Expression::Literal {
