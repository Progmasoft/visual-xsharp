...
{
  pub(super) fn lower_result_constructor(&mut self,
                                         target: mir::LocalId,
                                         ...
                                         lowered: &mut mir::Function)
  {
    ...
    self.current_block_mut(lowered)
        ...
        .push(mir::Statement::ConstBool { local: tag,
                                          ...
                                          span });
    let Some(inactive) = self.canonical_result_payload(if success_variant
                                                       {
                                                         error_type
                                                       }
                                                       else
  ...
  pub(super) fn lower_result_propagation(&mut self,
                                         expression: &Expression,
                                         expected_success_type: XlilType,
                                         lowered: &mut mir::Function)
                                         -> Option<mir::LocalId>
  {
    let Expression::ResultPropagation { value,
                                        span, } = expression
    else
    {
      return None;
    ...
    self.current_block_mut(lowered)
        .statements
        .push(mir::Statement::ConstBool { local: false_tag,
                                          value: false,
                                          span: *span });
    ...
    self.current_block_mut(lowered)
        .statements
        .push(mir::Statement::Aggregate { result: propagated,
  ...
  fn canonical_result_payload(&mut self,
                              value_type: XlilType,
                              span: Span,
                              lowered: &mut mir::Function)
                              -> Option<mir::LocalId>
  ...
  fn extract_result_field(&mut self,
