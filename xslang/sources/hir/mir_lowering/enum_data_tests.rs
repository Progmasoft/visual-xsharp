...
fn constructor(enum_type: &str,
               ...
               -> Expression
{
  Expression::EnumData { enum_type: enum_type.to_string(),
...
{
  enum_data("Value", &[], vec![variant("Number", Some(PrimitiveType::Int), 0),
...
{
  Expression::Object { nominal_type: "Point".to_string(),
                       fields: vec![ObjectField { name: "x".to_string(),
                                                  value: integer(x),
                                                  span: span() },
                                    ObjectField { name: "y".to_string(),
                                                  value: integer(y),
                                                  span: span() }],
                       span: span() }
}
...
fn lowers_int_payload_with_tag_and_inactive_long_zero()
{
  ...
  let function = returning("make_int",
                           constructor("Value",
                                       "Value",
                                       "Number",
                                       0,
  ...
  let lowered = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                      .lower_function(&function)
                                      .expect("typed enum data constructor should lower");
  assert_eq!(lowered.return_type, XlilType::aggregate(0));
  assert!(matches!(lowered.blocks[0].statements[0], mir::Statement::ConstI32 { value: 0,
                                                                               .. }));
...
{
  let declaration = value_declaration();
  let function = returning("make_long",
                           constructor("Value",
                                       "Value",
                                       "Number",
                                       1,
  ...
  let lowered = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                      .lower_function(&function)
                                      .unwrap();
  assert!(matches!(lowered.blocks[0].statements[0], mir::Statement::ConstI32 { value: 1,
                                                                               .. }));
  assert!(matches!(lowered.blocks[0].statements[1],
                   mir::Statement::ConstInteger { value: mir::IntegerConstant::I64(0),
                                                  .. }));
  assert!(matches!(lowered.blocks[0].statements[2], mir::Statement::ConstI32 { value: 7,
                                                                               .. }));
  assert!(crate::mir::verify::verify_function(&lowered).is_empty());
}
...
fn lowers_payload_free_variant_with_zeroed_payload_storage()
{
  ...
  let lowered = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                      .lower_function(&function)
                                      .unwrap();
  assert!(matches!(lowered.blocks[0].statements[0], mir::Statement::ConstI32 { value: 2,
                                                                               .. }));
  assert!(matches!(lowered.blocks[0].statements[1],
                   mir::Statement::ConstInteger { value: mir::IntegerConstant::I64(0),
                                                  .. }));
  assert!(matches!(lowered.blocks[0].statements[2],
                   mir::Statement::ConstInteger { value: mir::IntegerConstant::I32(0),
                                                  .. } |
                   mir::Statement::ConstI32 { value: 0,
                                              .. }));
}
...
fn rejects_payload_metadata_that_disagrees_with_selected_overload()
{
  ...
  let function = returning("bad",
                           constructor("Value",
                                       ...
                                       Some(integer("7")),
                                       Some(Type::Primitive(PrimitiveType::Long))));
  let diagnostics = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                          .lower_function(&function)
                                          .unwrap_err();
...
fn lowers_enum_data_mir_through_xlil_registry_and_verifier()
{
  ...
  {
    assert_eq!(module.add_aggregate_type(layout.name.clone(), layout.fields.clone()),
...
fn inherited_constructor_uses_flattened_derived_tag_and_payload_slot()
{
  ...
  let leaf = enum_data("Value", &["Root"], vec![variant("Number",
                                                        Some(PrimitiveType::Long),
                                                        0)]);
  let function = returning("make_root_number",
                           constructor("Value",
                                       "Root",
                                       "Number",
                                       0,
                                       Some(integer("9")),
                                       Some(Type::Primitive(PrimitiveType::Int))));
  let mir = HirToMirLowerer::new().with_nominal_types(&[root, leaf])
                                  .lower_function(&function)
                                  .unwrap();
  assert!(matches!(mir.blocks[0].statements[0], mir::Statement::ConstI32 { value: 0,
                                                                           .. }));
  assert!(matches!(mir.blocks[0].statements[1], mir::Statement::ConstI64 { value: 9,
                                                                           .. }));
...
{
  let declaration = value_declaration();
  let function = Function { name: "identity".to_string(),
                            return_type: Some(Type::Named("Value".to_string())),
                            locals: vec![Local { name: "value".to_string(),
                                                 ty: Type::Named("Value".to_string()),
                                                 mutable: false,
                                                 span: span() }],
                            body: vec![Statement::Return { value: Some(Expression::Local { name:
                                                                                             "value".to_string(),
                                                                                           span: span() }),
                                                           span: span() }] };
  let mir = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
...
fn nominal_data_payload_lowers_as_nested_aggregate_slot()
{
  ...
  let wrapped = NominalType { name: "Value".to_string(),
                              kind: NominalKind::EnumData,
                              bases: Vec::new(),
                              fields: Vec::new(),
                              variants: vec![EnumVariant { name: "Point".to_string(),
                                                           payload: Some(TypeRef::Named("Point".to_string())),
                                                           tag: 0,
                                                           span: source_span() },
                                             variant("Empty", None, 1)],
                              span: source_span() };
  let function = returning("wrap_point",
                           constructor("Value",
                                       "Value",
                                       "Point",
                                       0,
  ...
  let mir = HirToMirLowerer::new().with_nominal_types(&[point, wrapped])
                                  .lower_function(&function)
                                  .unwrap();
...
fn payload_free_variant_recursively_zeros_inactive_nominal_payload()
{
  ...
  let mir = HirToMirLowerer::new().with_nominal_types(&[point, wrapped])
                                  .lower_function(&function)
                                  .unwrap();
  let zero_count = mir.blocks[0].statements
                                .iter()
                                .filter(|statement| {
                                  matches!(statement,
                                           mir::Statement::ConstInteger { value: mir::IntegerConstant::I32(0),
...
fn enum_data_local_binding_stores_and_loads_the_complete_value()
{
  ...
  let function = Function { name: "local_flow".to_string(),
                            ...
                            body: vec![Statement::Let { local: Local { name: "value".to_string(),
                                                                       ...
                                                                       mutable: false,
                                                                       span: span() },
                                                        initializer: Some(constructor("Value", "Value",
                                                                                      "Empty", 2, None, None)) },
...
{
  let declaration = enum_data("Value", &[], vec![variant("Value", Some(PrimitiveType::Bool), 0),
                                                 variant("Value", Some(PrimitiveType::Float), 1)]);
  let function = returning("wrap_bool",
                           constructor("Value",
                                       "Value",
                                       "Value",
                                       0,
                                       Some(Expression::Literal { literal: Literal::Bool(true),
                                                                  span: span() }),
  ...
  assert!(mir.blocks[0].statements
                       .iter()
                       .any(|statement| matches!(statement, mir::Statement::ConstBool { value: true,
                                                                                        .. })));
  assert!(mir.blocks[0].statements
                       .iter()
                       .any(|statement| matches!(statement, mir::Statement::ConstF64 { bits: 0,
                                                                                       .. })));
  assert!(crate::mir::verify::verify_function(&mir).is_empty());
}
...
{
  let declaration = enum_data("Value", &[], vec![variant("Value", Some(PrimitiveType::Float), 0),
                                                 variant("Value", Some(PrimitiveType::Str), 1)]);
  let function = returning("wrap_float",
                           constructor("Value",
                                       "Value",
                                       "Value",
                                       0,
                                       Some(Expression::Literal { literal: Literal::Float("3.5".to_string()),
                                                                  span: span() }),
...
{
  let declaration = enum_data("Value", &[], vec![variant("Half", Some(PrimitiveType::SFloat), 0),
                                                 variant("Empty", None, 1)]);
  let function = returning("empty", constructor("Value", "Value", "Empty", 1, None, None));
  let diagnostics = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                          .lower_function(&function)
...
{
  let declaration = enum_data("Value", &[], vec![variant("Wide", Some(PrimitiveType::Double), 0),
                                                 variant("Empty", None, 1)]);
  let function = returning("empty", constructor("Value", "Value", "Empty", 1, None, None));
  let diagnostics = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                          .lower_function(&function)
...
{
  let declaration = enum_data("Value", &[], vec![variant("Text", Some(PrimitiveType::String), 0),
                                                 variant("Empty", None, 1)]);
  let function = returning("empty", constructor("Value", "Value", "Empty", 1, None, None));
  let diagnostics = HirToMirLowerer::new().with_nominal_types(std::slice::from_ref(&declaration))
                                          .lower_function(&function)
...
{
  let first = enum_data("Value", &[], vec![variant("Number", Some(PrimitiveType::Long), 0),
                                           variant("Empty", None, 1)]);
  let second = enum_data("Other", &[], vec![variant("Number", Some(PrimitiveType::Long), 0),
                                            variant("Empty", None, 1)]);
  let function = Function { name: "bad".to_string(),
                            return_type: Some(Type::Named("Other".to_string())),
                            locals: Vec::new(),
                            body: vec![Statement::Return { value: Some(constructor("Value", "Value", "Empty",
                                                                                   1, None, None)),
                                                           span: span() }] };
  let diagnostics = HirToMirLowerer::new().with_nominal_types(&[first, second])
                                          .lower_function(&function)
                                          .unwrap_err();
  assert!(diagnostics.iter()
                     .any(|diagnostic| diagnostic.code == DiagnosticCode::UnsupportedType));
