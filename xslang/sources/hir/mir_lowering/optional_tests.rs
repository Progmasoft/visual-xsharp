...
fn lower(function: &Function) -> (mir::Function, crate::xlil::Module)
{
    ...
    let registry = aggregate_registry::build_functions_with_nominals(&[], std::slice::from_ref(function))
        .expect("Optional<Long> registry should build");
...
fn lowers_some_coalesce_through_xhir_xmir_and_xlil()
{
    ...
    assert!(matches!(&parsed_xhir.body[1], Statement::Return {
        value: Some(Expression::Binary {
            operator: BinaryOperator::Coalesce,
            ..
        }),
    ...
    assert_eq!(
        crate::mir::text::function_to_xmir(&crate::mir::text::parse_xmir_function(&xmir).expect("XMIR should parse")),
        xmir
    );
...
{
    let function = function(Expression::Literal {
    ...
    assert!(
        module.functions[0]
            .blocks
            .iter()
            .any(|block| { matches!(block.terminator, Some(crate::xlil::Terminator::BranchIf { .. })) })
    );
}
...
{
    let function = updating_function(
        Expression::Literal {
            literal: Literal::None,
            span: span(),
        },
...
fn lowers_none_coalescing_assignment_and_unwrap_through_xlil()
{
    ...
    assert!(xmir.contains("statement store.local"));
    assert_eq!(
        crate::mir::text::function_to_xmir(
            &crate::mir::text::parse_xmir_function(&xmir).expect("Optional XMIR should parse")
        ),
        xmir
    );
    ...
    let parsed = crate::xlil::parse_module(&xlil)
        .unwrap_or_else(|diagnostics| panic!("Optional XLIL should parse:\n{xlil}\n{diagnostics:#?}"));
    assert!(crate::xlil::verify_module(&parsed).is_empty());
    assert_eq!(crate::xlil::module_to_string(&parsed), xlil);
}
...
fn rejects_forced_unwrap_of_a_non_optional_value()
{
    let invalid = Function {
        ...
        locals: vec![],
        body: vec![Statement::Return {
            value: Some(Expression::OptionalUnwrap {
...
{
    let mut invalid = updating_function(
        Expression::Literal {
            literal: Literal::None,
            span: span(),
        },
    ...
    assert!(
        diagnostics
            .iter()
            .any(|diagnostic| { diagnostic.code == crate::hir::type_check::DiagnosticCode::ImmutableAssignment })
    );
}
...
fn rejects_a_coalescing_assignment_with_the_wrong_payload_type()
{
    let invalid = updating_function(
        ...
        Expression::Call {
            ...
            return_type: Box::new(Type::Optional {
