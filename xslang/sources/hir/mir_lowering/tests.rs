...
fn lowers_int_literal_return_to_const_i64()
{
    ...
    assert!(matches!(mir.blocks[0].statements[0], mir::Statement::ConstI64 {
        value: 42,
        ..
    }));
    assert_eq!(
...
{
    let function = Function {
        name: "LongAnswer".to_string(),
        return_type: Some(primitive(PrimitiveType::Long)),
        locals: vec![],
        body: vec![Statement::Return {
            value: Some(Expression::Literal {
                literal: Literal::Integer("2'000'000'000".to_string()),
                span: span(10, 23),
            }),
            span: span(3, 23),
        }],
    ...
    assert!(matches!(
        xlil.blocks[0].instructions[0],
        crate::xlil::Instruction::ConstI32 {
            value: 2_000_000_000,
            ..
        }
    ));
...
{
    let function = Function {
        name: "TooLarge".to_string(),
        return_type: Some(primitive(PrimitiveType::Long)),
        locals: vec![],
        body: vec![Statement::Return {
            value: Some(Expression::Literal {
                literal: Literal::Integer("2'147'483'648".to_string()),
                span: span(10, 23),
            }),
            span: span(3, 23),
        }],
...
fn lowers_int_local_initializer_and_return()
{
    let function = Function {
        ...
        body: vec![
            Statement::Let {
                local: local("answer", primitive(PrimitiveType::Int), false),
                initializer: Some(Expression::Literal {
                    literal: Literal::Integer("1'024".to_string()),
                    span: span(10, 15),
                }),
            },
            Statement::Return {
    ...
    assert!(
        mir.blocks[0]
            .statements
            .iter()
            .any(|statement| matches!(statement, mir::Statement::ConstI64 {
                value: 1024,
                ..
            }))
    );
...
{
    let function = Function {
        name: "AddLong".to_string(),
        return_type: Some(primitive(PrimitiveType::Long)),
        locals: vec![],
        body: vec![
            Statement::Let {
                local: local("left", primitive(PrimitiveType::Long), false),
                initializer: Some(Expression::Literal {
                    literal: Literal::Integer("4".to_string()),
                    span: span(1, 2),
                }),
            },
            Statement::Let {
                local: local("right", primitive(PrimitiveType::Long), false),
                initializer: Some(Expression::Literal {
