...
{
    pub(super) fn lower_literal_into(
        &mut self,
        target: mir::LocalId,
        literal: &Literal,
        span: Span,
    ...
    )
    {
        ...
        match (literal, value_type)
        {
            ...
            ) =>
            {
                self.current_block_mut(lowered)
                    .statements
                    .push(mir::Statement::ConstInteger {
                        local: target,
    ...
    pub(super) fn lower_optional_none(
        &mut self,
        target: mir::LocalId,
    ...
    pub(super) fn lower_default_value(
        &mut self,
        target: mir::LocalId,
        value_type: XlilType,
        span: Span,
    ...
    ) -> bool
    {
        ...
        {
            XlilType::BOOL => Some(mir::Statement::ConstBool {
                local: target,
                value: false,
                span,
            }),
            XlilType::F32 => Some(mir::Statement::ConstF32 {
                local: target,
                bits: 0,
                span,
            }),
            XlilType::F64 => Some(mir::Statement::ConstF64 {
                local: target,
                bits: 0,
                span,
            }),
            XlilType::STR => Some(mir::Statement::ConstStr {
                local: target,
                units: Vec::new(),
                span,
            }),
    ...
    pub(super) fn declare_storage_temp(
        &mut self,
        value_type: XlilType,
        span: Span,
        lowered: &mut mir::Function,
    ...
    pub(super) fn lower_value_type(&mut self, ty: &Type, span: Span) -> Option<XlilType>
    {
        match ty
        {
            Type::Named(name) => self.aggregate_types.get(name).copied().or_else(|| {
                self.report(
                    DiagnosticCode::UnsupportedType,
                    format!("nominal HIR type '{name}' has no aggregate MIR layout"),
                    span,
                );
                None
            }),
            ...
            Type::Reference {
                referent,
                mutable: false,
            } if **referent == Type::Primitive(PrimitiveType::Str) => Some(XlilType::STR),
            Type::Reference {
                ..
            } =>
            {
