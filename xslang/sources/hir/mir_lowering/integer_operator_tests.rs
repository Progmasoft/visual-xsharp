...
fn binary_function(primitive: PrimitiveType, operator: BinaryOperator, comparison: bool) -> Function
{
    ...
    Function {
        name: "integer_operator".to_string(),
...
{
    let cases = [
        (PrimitiveType::Byte, XlilType::U8),
    ...
    for (primitive, value_type) in cases
    {
        ...
        let parsed =
            parse_xhir_function(&function_to_xhir(&function)).expect("integer operator XHIR should round-trip");
        let mir = HirToMirLowerer::new()
            .lower_function(&parsed)
            .expect("integer operator should lower to MIR");
        let mir = crate::mir::text::parse_xmir_function(&crate::mir::text::function_to_xmir(&mir))
            .expect("integer operator XMIR should round-trip");
      ...
      matches!(statement, mir::Statement::BinaryInteger { operation: IntegerBinaryOperation::Add, value_type: actual, .. } if *actual == value_type)
        ...
        let reparsed = crate::xlil::parse_module(&crate::xlil::module_to_string(&module))
            .expect("integer operator XLIL should round-trip");
...
{
    for (primitive, value_type) in [
        (PrimitiveType::UInteger, XlilType::U128),
        (PrimitiveType::Integer, XlilType::I128),
    ]
    {
        for (operator, operation, comparison) in [
            (BinaryOperator::Div, IntegerBinaryOperation::Div, false),
