...
{
    pub(super) fn lower_member_value(
        &mut self,
    ...
    pub(super) fn lower_object_value(
        &mut self,
        type_name: &str,
    ...
    pub(super) fn lower_nominal_parameter(&mut self, local: &Local, lowered: &mut mir::Function)
    {
        ...
        self.append_nominal_parameters(&local.name, &[], &definition, local.span, lowered, &mut vec![
            definition.name.clone(),
        ]);
    }

    pub(super) fn lower_nominal_binding(
        &mut self,
        local: &Local,
    ...
    pub(super) fn lower_nominal_place_value(
        &mut self,
        root: &str,
        type_name: &str,
        span: Span,
    ...
    ) -> Option<mir::LocalId>
    {
        ...
        for field in fields
        {
            ...
            {
                Type::Primitive(_) => self.lower_field_load(
                    &FieldPath {
                        root: root.to_string(),
                        fields: path,
                        ty: field_type,
    ...
    fn declare_nominal_place_fields(
        &mut self,
        root: &str,
        prefix: &[String],
        definition: &crate::hir::declarations::NominalType,
        ...
        if let Type::Named(type_name) = &target.ty
        {
            ...
            else
            {
                self.report(
                    DiagnosticCode::UnknownLocal,
                    format!(
                        "nominal field place '{}' has not been initialized",
    ...
    pub(super) fn lower_nominal_root_assignment(
        &mut self,
        root: &str,
        type_name: &str,
        value: &Expression,
    ...
    pub(super) fn lower_nominal_argument(
        &mut self,
        expression: &Expression,
        type_name: &str,
        lowered: &mut mir::Function,
    ) -> Option<Vec<mir::LocalId>>
    {
        ...
        )
        {
            let value_type = self.aggregate_types.get(type_name).copied()?;
            return self
                .lower_expression_to_local(expression, value_type, lowered)
                .map(|value| vec![value]);
        }
        if definition.kind != crate::hir::declarations::NominalKind::Data
        {
        ...
        match expression
        {
            ...
            } =>
            {
                ...
                self.append_nominal_place_arguments(name, &[], &definition, *span, lowered, &mut NominalArguments {
            ...
            Expression::Field {
                path,
            } if path.ty == Type::Named(type_name.to_string()) =>
            {
                let mut visiting = vec![definition.name.clone()];
                self.append_nominal_place_arguments(
                    &path.root,
                    &path.fields,
                    &definition,
                    path.span,
                    lowered,
                    &mut NominalArguments {
                        values: &mut arguments,
                        visiting: &mut visiting,
                    },
                )?;
            ...
            {
                self.append_nominal_object_arguments(&definition, fields, *span, lowered, &mut arguments, &mut vec![
                    definition.name.clone(),
                ])?;
            }
            _ =>
            {
                ...
                self.append_nominal_aggregate_arguments(
                    &definition,
                    aggregate,
                    span,
                    lowered,
