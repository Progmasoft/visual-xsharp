...
{
  declaration("Value", &[], vec![variant("Number", Some(PrimitiveType::Long), 0),
                                 variant("Wide", Some(PrimitiveType::Int), 1),
                                 variant("Empty", None, 2)])
}

fn pattern(owner: &str,
           variant: &str,
           tag: u32,
           binding: Option<&str>,
           ...
           -> MatchPattern
{
  MatchPattern::EnumDataVariant { enum_type: "Value".to_string(),
...
{
  MatchArm { pattern,
             body: Block { statements: vec![Statement::Return { value: Some(value),
                                                                span: span() }],
                           tail: None,
                           span: span() },
             span: span() }
...
{
  let match_value = Expression::Match { selector: Box::new(local("input")),
  ...
  Function { name: name.to_string(),
             ...
             locals: vec![crate::hir::Local { name: "input".to_string(),
                                              ty: Type::Named("Value".to_string()),
                                              mutable: false,
                                              span: span() }],
             body: if expression
             {
               vec![Statement::Return { value: Some(match_value),
                                        span: span() }]
             }
             else
             {
               vec![Statement::Match { selector: local("input"),
...
{
  HirToMirLowerer::new().with_nominal_types(declarations)
...
fn lower_to_module(function: &Function, declarations: &[NominalType]) -> crate::xlil::Module
{
  ...
  let function = crate::xlil::lowering::MirToXlilLowerer::new().lower_function(&mir)
...
fn lowers_enum_data_tag_test_to_extract_compare_and_branch()
{
  ...
  let function = function("select",
                          vec![returning(pattern("Value", "Number", 0, None, Some(PrimitiveType::Long)),
                                         integer("7")),
                               returning(MatchPattern::Else, integer("9"))],
                          false);
  let mir = lower(&function, &[declaration]);
  let statements = &mir.blocks[0].statements;
  assert!(matches!(statements[0], mir::Statement::Extract { field: 0,
                                                            field_type: XlilType::I32,
                                                            .. }));
  assert!(statements.iter()
                    .any(|statement| matches!(statement, mir::Statement::ConstI32 { value: 0,
                                                                                    .. })));
  assert!(statements.iter()
...
fn lowers_payload_binding_from_the_selected_slot()
{
  ...
  let function = function("unwrap",
                          vec![returning(pattern("Value", "Number", 0, Some("number"), Some(PrimitiveType::Long)),
                                         local("number")),
...
{
  let declaration = value_declaration();
  let function = function("unwrap_wide",
                          vec![returning(pattern("Value", "Wide", 1, Some("wide"), Some(PrimitiveType::Int)),
                                         Expression::Binary { operator: BinaryOperator::Equal,
...
fn exhaustive_match_uses_the_final_variant_as_fallthrough()
{
  ...
  let function = function("all",
                          vec![returning(pattern("Value", "Number", 0, None, Some(PrimitiveType::Long)),
                                         integer("1")),
                               returning(pattern("Value", "Wide", 1, None, Some(PrimitiveType::Int)),
                                         integer("2")),
                               returning(pattern("Value", "Empty", 2, None, None), integer("3"))],
                          false);
  let mir = lower(&function, &[declaration]);
  let branches = mir.blocks
                    .iter()
...
fn match_expression_merges_bound_payload_value()
{
  ...
  let function = function("value",
                          vec![yielding(pattern("Value", "Number", 0, Some("number"), Some(PrimitiveType::Long)),
                                        local("number")),
...
fn inherited_variant_uses_flattened_tag_and_payload_slot()
{
  ...
  let value = declaration("Value", &["Root"], vec![variant("Text",
                                                           Some(PrimitiveType::String),
                                                           4)]);
  let function = function("inherited",
                          vec![returning(pattern("Root", "Number", 0, Some("number"), Some(PrimitiveType::Long)),
                                         local("number")),
                               returning(pattern("Value", "Text", 1, None, Some(PrimitiveType::String)),
                                         integer("0"))],
                          false);
...
{
  let declaration = value_declaration();
  let function = function("bad",
                          vec![returning(pattern("Other", "Number", 99, None, Some(PrimitiveType::Long)),
                                         integer("1")),
                               returning(MatchPattern::Else, integer("0"))],
                          false);
  let diagnostics = HirToMirLowerer::new().with_nominal_types(&[declaration])
                                          .lower_function_with_parameters(&function, 1)
...
fn payload_free_pattern_does_not_extract_an_inactive_payload_slot()
{
  ...
  let function = function("empty",
                          vec![returning(pattern("Value", "Empty", 2, None, None), integer("1")),
                               returning(MatchPattern::Else, integer("0"))],
                          false);
  let mir = lower(&function, &[declaration]);
  let extracts = mir.blocks
                    .iter()
                    .flat_map(|block| &block.statements)
