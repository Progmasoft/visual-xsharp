...
{
  pub(super) fn lower_optional_member(&mut self,
                                      expression: &Expression,
                                      result_type: XlilType,
                                      lowered: &mut mir::Function)
                                      -> Option<mir::LocalId>
  {
    let Expression::OptionalMember { receiver,
                                     owner,
                                     name,
                                     field_type,
    ...
    let receiver_type = self.optional_layouts
                            .iter()
    ...
    self.set_terminator(mir::Terminator::BranchIf { condition: receiver_tag,
                                                    then_block: present,
                                                    else_block: absent },
                        *span,
                        lowered);
    ...
    let field_value = self.extract_optional_field(owner_value,
    ...
    self.current_block_mut(lowered)
        .statements
        .push(mir::Statement::ConstBool { local: true_tag,
                                          value: true,
                                          span: *span });
    let some = self.declare_temp(result_type, *span, lowered)?;
    self.current_block_mut(lowered)
        .statements
        .push(mir::Statement::Aggregate { result: some,
  ...
  pub(super) fn lower_optional_unwrap(&mut self,
                                      optional: &Expression,
                                      result_type: XlilType,
                                      span: Span,
                                      lowered: &mut mir::Function)
                                      -> Option<mir::LocalId>
  {
    let optional_type = self.optional_layouts
                            .iter()
  ...
  pub(super) fn lower_optional_coalesce_assign(&mut self,
                                               target: &str,
                                               value: &Expression,
  ...
  fn extract_optional_field(&mut self,
