...
{
  Base { ty: TypeRef::Named(name.to_string()),
...
{
  vec![variant("Number", Some(primitive(PrimitiveType::Int)), 0),
...
{
  let registry = EnumDataRegistry::build(&[enum_data("Value", &[], number_variants())]);
  let int = registry.select("Value", "Number", Some(&Type::Primitive(PrimitiveType::Int)))
                    .unwrap();
  let long = registry.select("Value", "Number", Some(&Type::Primitive(PrimitiveType::Long)))
                     .unwrap();
  assert_eq!(int.tag, 0);
...
{
  let registry = EnumDataRegistry::build(&[enum_data("Value", &[], number_variants())]);
  assert_eq!(registry.select("Value", "Number", None),
             Err(VariantSelectionError::PayloadRequired { type_name: "Value".to_string(),
...
{
  let registry = EnumDataRegistry::build(&[enum_data("Value", &[], number_variants())]);
  assert_eq!(registry.select("Value", "Empty", Some(&Type::Primitive(PrimitiveType::Int))),
             Err(VariantSelectionError::UnexpectedPayload { type_name: "Value".to_string(),
...
{
  let registry = EnumDataRegistry::build(&[enum_data("Value", &[], number_variants())]);
  assert_eq!(registry.select("Value", "Number", Some(&Type::Primitive(PrimitiveType::Integer))),
             Err(VariantSelectionError::NoMatchingPayload { type_name: "Value".to_string(),
                                                            variant: "Number".to_string(),
                                                            actual: Type::Primitive(PrimitiveType::Integer),
                                                            candidates: vec![primitive(PrimitiveType::Int),
...
fn reports_unknown_type_and_regular_enum()
{
  ...
  assert_eq!(registry.variants("Missing"),
             Err(VariantSelectionError::UnknownType("Missing".to_string())));
  assert_eq!(registry.variants("Color"),
...
{
  let registry = EnumDataRegistry::build(&[enum_data("Value", &[], number_variants())]);
  assert_eq!(registry.select("Value", "Missing", None),
             Err(VariantSelectionError::UnknownVariant { type_name: "Value".to_string(),
...
{
  let declarations = [enum_data("Root", &[], vec![variant("RootValue",
                                                          Some(primitive(PrimitiveType::Int)),
                                                          0)]),
                      enum_data("Middle", &["Root"], vec![variant("MiddleValue",
                                                                  Some(primitive(PrimitiveType::Long)),
                                                                  0)]),
                      enum_data("Leaf",
                                &["Middle"],
                                vec![variant("LeafValue", Some(primitive(PrimitiveType::Integer)), 0)])];
  let registry = EnumDataRegistry::build(&declarations);
  let variants = registry.variants("Leaf").unwrap();
  assert_eq!(variants.iter().map(|value| value.name.as_str()).collect::<Vec<_>>(),
             ["RootValue", "MiddleValue", "LeafValue"]);
...
{
  let declarations = [enum_data("Base", &[], vec![variant("Number",
                                                          Some(primitive(PrimitiveType::Int)),
                                                          0)]),
                      enum_data("Derived",
                                &["Base"],
                                vec![variant("Number", Some(primitive(PrimitiveType::Long)), 0)])];
  let registry = EnumDataRegistry::build(&declarations);
...
{
  let declarations = [enum_data("Left", &[], vec![variant("LeftValue",
                                                          Some(primitive(PrimitiveType::Int)),
                                                          0)]),
                      enum_data("Right", &[], vec![variant("RightValue",
                                                           Some(primitive(PrimitiveType::Long)),
                                                           0)]),
                      enum_data("Combined",
                                &["Right", "Left"],
                                vec![variant("Own", Some(primitive(PrimitiveType::Integer)), 0)])];
  let registry = EnumDataRegistry::build(&declarations);
...
fn deduplicates_a_shared_ancestor_in_a_diamond()
{
  let declarations = [enum_data("Root", &[], vec![variant("RootValue",
                                                          Some(primitive(PrimitiveType::Int)),
                                                          0)]),
                      enum_data("Left", &["Root"], vec![variant("LeftValue",
                                                                Some(primitive(PrimitiveType::Long)),
                                                                0)]),
                      enum_data("Right",
                                &["Root"],
                                vec![variant("RightValue", Some(primitive(PrimitiveType::Integer)), 0)]),
                      enum_data("Diamond",
                                &["Left", "Right"],
                                vec![variant("Own", Some(primitive(PrimitiveType::Short)), 0)])];
  let registry = EnumDataRegistry::build(&declarations);
  let variants = registry.variants("Diamond").unwrap();
  assert_eq!(variants.iter().filter(|value| value.owner == "Root").count(), 1);
  assert_eq!(variants.iter().map(|value| value.name.as_str()).collect::<Vec<_>>(),
             ["RootValue", "LeftValue", "RightValue", "Own"]);
}

#[test]
fn rejects_duplicate_typed_payload_in_one_declaration()
{
  let declaration = enum_data("Value", &[], vec![variant("Number",
                                                         Some(primitive(PrimitiveType::Int)),
                                                         0),
                                                 variant("Number",
                                                         Some(primitive(PrimitiveType::Int)),
  ...
  assert_eq!(registry.diagnostics(),
             [EnumDataDiagnostic { type_name: "Value".to_string(),
                                   error: EnumDataError::DuplicatePayload { name: "Number".to_string(),
                                                                            payload:
...
{
  let declarations = [enum_data("Left", &[], vec![variant("Number",
                                                          Some(primitive(PrimitiveType::Int)),
                                                          0)]),
                      enum_data("Right", &[], vec![variant("Number",
                                                           Some(primitive(PrimitiveType::Int)),
                                                           0)]),
                      enum_data("Combined",
                                &["Left", "Right"],
                                vec![variant("Own", Some(primitive(PrimitiveType::Long)), 0)])];
  let registry = EnumDataRegistry::build(&declarations);
  assert!(registry.diagnostics().iter().any(|diagnostic| {
                                         diagnostic.type_name == "Combined" &&
                                         matches!(diagnostic.error, EnumDataError::DuplicatePayload { .. })
                                       }));
}
...
{
  let declaration = enum_data("Value", &[], vec![variant("Some",
                                                         Some(primitive(PrimitiveType::Int)),
                                                         0),
                                                 variant("None", None, 1),
                                                 variant("None", None, 2)]);
  let registry = EnumDataRegistry::build(&[declaration]);
  assert_eq!(registry.diagnostics()[0].error,
...
{
  let declaration = enum_data("Value", &[], vec![variant("Item", None, 0),
                                                 variant("Item",
                                                         Some(primitive(PrimitiveType::Int)),
                                                         1)]);
  let registry = EnumDataRegistry::build(&[declaration]);
...
fn rejects_enum_data_without_a_direct_typed_variant()
{
  ...
  let registry = EnumDataRegistry::build(&[declaration]);
  assert_eq!(registry.diagnostics()[0], EnumDataDiagnostic { type_name:
                                                               "EmptyLike".to_string(),
                                                             error:
                                                               EnumDataError::MissingTypedVariant });
}

#[test]
fn rejects_unknown_base()
{
  let declaration = enum_data("Value",
                              &["Missing"],
                              vec![variant("Own", Some(primitive(PrimitiveType::Int)), 0)]);
  let registry = EnumDataRegistry::build(&[declaration]);
  assert_eq!(registry.diagnostics()[0].error,
...
{
  let declarations = [nominal("Color", NominalKind::Enum),
                      enum_data("Value", &["Color"], vec![variant("Own",
                                                                  Some(primitive(PrimitiveType::Int)),
                                                                  0)])];
  let registry = EnumDataRegistry::build(&declarations);
  assert_eq!(registry.diagnostics()[0].error,
...
{
  let mut declaration = enum_data("Value", &[], vec![variant("Own",
                                                             Some(primitive(PrimitiveType::Int)),
                                                             0)]);
...
{
  let declarations = [enum_data("Left", &["Right"], vec![variant("LeftValue",
                                                                 Some(primitive(PrimitiveType::Int)),
                                                                 0)]),
                      enum_data("Right", &["Left"], vec![variant("RightValue",
                                                                 Some(primitive(PrimitiveType::Long)),
                                                                 0)])];
  let registry = EnumDataRegistry::build(&declarations);
...
{
  let declaration = enum_data("Event", &[], vec![variant("Data",
                                                         Some(TypeRef::Named("User".to_string())),
                                                         0),
                                                 variant("Data",
                                                         Some(TypeRef::Named("Message".to_string())),
                                                         1)]);
  let registry = EnumDataRegistry::build(&[declaration]);
  let user = registry.select("Event", "Data", Some(&Type::Named("User".to_string())))
                     .unwrap();
  let message = registry.select("Event", "Data", Some(&Type::Named("Message".to_string())))
                        .unwrap();
...
fn nominal_payload_selection_does_not_use_structural_compatibility()
{
  let declaration = enum_data("Event", &[], vec![variant("Data",
                                                         ...
                                                         0)]);
  let registry = EnumDataRegistry::build(&[declaration]);
  assert!(matches!(registry.select("Event", "Data", Some(&Type::Named("OtherUser".to_string()))),
...
{
  let optional = TypeRef::Optional { element: Box::new(primitive(PrimitiveType::Int)) };
  let result = TypeRef::Result { success: Box::new(primitive(PrimitiveType::Int)),
                                 error: Box::new(TypeRef::Named("Error".to_string())) };
  let declaration = enum_data("Container", &[], vec![variant("Value", Some(optional.clone()), 0),
                                                     variant("Value", Some(result.clone()), 1)]);
  let registry = EnumDataRegistry::build(&[declaration]);
  let optional_type = Type::Optional { element: Box::new(Type::Primitive(PrimitiveType::Int)) };
  let result_type = Type::Result { success: Box::new(Type::Primitive(PrimitiveType::Int)),
                                   error: Box::new(Type::Named("Error".to_string())) };
  assert_eq!(registry.select("Container", "Value", Some(&optional_type))
                     .unwrap()
                     .payload,
             Some(optional));
  assert_eq!(registry.select("Container", "Value", Some(&result_type))
                     .unwrap()
                     .payload,
             Some(result));
}
