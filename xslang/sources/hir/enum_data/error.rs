...
impl fmt::Display for EnumDataError
{
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result
    {
        match self
        {
            ...
            {
                write!(formatter, "contains an enum data inheritance cycle through '{name}'")
            }
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result
    {
        match self
        {
            ...
            Self::UnknownVariant {
                type_name,
                variant,
            } => write!(formatter, "enum data '{type_name}' has no variant '{variant}'"),
            ...
            Self::UnexpectedPayload {
                ...
                formatter,
                "enum data variant '{type_name}::{variant}' does not accept a payload"
            ),
            Self::NoMatchingPayload {
                type_name,
                variant,
                actual,
                candidates,
                ...
                formatter,
                "enum data variant '{type_name}::{variant}' has no overload for {actual:?}; candidates: {candidates:?}"
            ),
        }
    }
}
