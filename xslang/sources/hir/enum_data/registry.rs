...
impl EnumDataRegistry
{
  ...
  pub fn build(declarations: &[NominalType]) -> Self
  {
    let definitions = declarations.iter()
                                  ...
                                  .collect::<HashMap<_, _>>();
    let mut registry = Self { definitions,
    ...
    for declaration in declarations.iter().filter(|value| value.kind == NominalKind::EnumData)
    {
      if let Err(error) = registry.resolve_type(&declaration.name, &mut Vec::new())
      {
        registry.diagnostics
                .push(EnumDataDiagnostic { type_name: declaration.name.clone(),
                                           error });
      }
    }
    registry
  ...
  {
    let named = self.variants(type_name)?
                    .iter()
                    .filter(|candidate| candidate.name == variant_name)
                    .collect::<Vec<_>>();
    if named.is_empty()
    {
      return Err(VariantSelectionError::UnknownVariant { type_name: type_name.to_string(),
    ...
    match payload
    {
      None => named.iter()
                   ...
                   .ok_or_else(|| VariantSelectionError::PayloadRequired { type_name: type_name.to_string(),
                                                                           variant: variant_name.to_string() }),
      Some(actual) =>
      {
        let typed = named.iter()
                         .copied()
                         .filter(|candidate| candidate.payload.is_some())
                         .collect::<Vec<_>>();
        if typed.is_empty()
        {
          return Err(VariantSelectionError::UnexpectedPayload { type_name: type_name.to_string(),
                                                                variant: variant_name.to_string() });
        }
        typed.iter()
             .copied()
             .find(|candidate| candidate.payload.as_ref().and_then(type_ref_to_checked).as_ref() == Some(actual))
             .ok_or_else(|| VariantSelectionError::NoMatchingPayload { type_name: type_name.to_string(),
                                                                       variant: variant_name.to_string(),
                                                                       actual: actual.clone(),
                                                                       candidates: typed.iter()
  ...
  pub fn overloads(&self, type_name: &str, variant_name: &str) -> Result<Vec<&ResolvedVariant>, VariantSelectionError>
  {
    let variants = self.variants(type_name)?;
    let overloads = variants.iter()
                            .filter(|candidate| candidate.name == variant_name)
                            .collect::<Vec<_>>();
    if overloads.is_empty()
    {
      Err(VariantSelectionError::UnknownVariant { type_name: type_name.to_string(),
                                                  variant: variant_name.to_string() })
    }
    else
    {
  ...
  fn resolve_type(&mut self, type_name: &str, stack: &mut Vec<String>) -> Result<Vec<ResolvedVariant>, EnumDataError>
  {
  ...
  fn collect_definition(&mut self,
                        ...
                        -> Result<Vec<ResolvedVariant>, EnumDataError>
  {
    ...
    variants.extend(definition.variants
                              .iter()
...
fn validate_overloads(variants: &[ResolvedVariant]) -> Result<(), EnumDataError>
{
  ...
  for (name, overloads) in by_name
  {
    ...
    for variant in overloads
    {
      ...
      {
        return Err(EnumDataError::DuplicatePayload { name: name.to_string(),
