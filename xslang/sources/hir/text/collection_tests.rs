...
fn roundtrips_builtin_array_and_map_records()
{
    ...
    let map_type = Type::Map {
        key: Box::new(Type::Primitive(PrimitiveType::Str)),
    ...
    assert_eq!(
        parse_xhir_function(&text).expect("collection XHIR should parse"),
        function
    );
}
...
{
    let function = Function {
        name: "mutate_array".to_string(),
        return_type: None,
        locals: Vec::new(),
        body: vec![Statement::AssignIndex {
    ...
    assert_eq!(
        parse_xhir_function(&text).expect("array assignment XHIR should parse"),
        function
    );
}
