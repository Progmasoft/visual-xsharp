...
) -> Vec<NominalType>
{
    ...
    *index += 1;
    ...
    diagnostics.push(error(
        lines.len().max(1),
        "unterminated XHIR declarations section".to_string(),
    ));
    declarations
}
...
) -> (Vec<Base>, Vec<Field>, Vec<EnumVariant>)
{
    ...
    diagnostics.push(error(
        lines.len().max(1),
        "unterminated nominal declaration".to_string(),
    ));
...
{
    crate::hir::declarations::type_ref_to_checked(ty)
        .map(|ty| type_name(&ty))
...
fn checked_to_ref(ty: Type) -> Option<TypeRef>
{
    Some(match ty
    {
        ...
        Type::Optional {
            ...
            element: Box::new(checked_to_ref(*element)?),
        },
        Type::Result {
            success,
            error,
        } => TypeRef::Result {
            success: Box::new(checked_to_ref(*success)?),
            error: Box::new(checked_to_ref(*error)?),
        },
        Type::Reference {
            referent,
            mutable,
        } => TypeRef::Reference {
            referent: Box::new(checked_to_ref(*referent)?),
            mutable,
        },
        Type::Array {
            element,
        ...
        Type::Map {
            ...
            key: Box::new(checked_to_ref(*key)?),
