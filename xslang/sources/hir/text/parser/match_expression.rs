...
impl Parser<'_>
{
    pub(super) fn match_pattern(&mut self, line: &str) -> Option<MatchPattern>
    {
        ...
        if let Some(enum_data) = record.strip_prefix("enum_data ")
        {
            ...
            return Some(MatchPattern::EnumDataVariant {
                enum_type: enum_type.to_string(),
                owner: owner.to_string(),
                variant: variant.to_string(),
                tag,
                binding: (binding != "else").then(|| binding.to_string()),
                payload_type: (payload_type != "()").then(|| self.parse_type(payload_type)).flatten(),
            });
        }
        ...
        Some(MatchPattern::ResultVariant {
            success,
            binding: (binding != "else").then(|| binding.to_string()),
