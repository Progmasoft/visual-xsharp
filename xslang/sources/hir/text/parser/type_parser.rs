...
pub(crate) fn parse_type_text(name: &str) -> Option<Type>
{
    if let Some(referent) = name.strip_prefix("&mut ")
    {
        return Some(Type::Reference {
            referent: Box::new(parse_type_text(referent.trim())?),
            mutable: true,
        });
    }
    ...
    {
        return Some(Type::Optional {
            element: Box::new(parse_type_text(element.trim())?),
        });
    }
    if let Some(body) = name.strip_prefix("Result<").and_then(|value| value.strip_suffix('>'))
    {
        ...
        if let [success, error] = parts.as_slice()
        {
            return Some(Type::Result {
                success: Box::new(parse_type_text(success.trim())?),
    ...
    if let Some(body) = name.strip_prefix('(').and_then(|value| value.strip_suffix(')'))
    {
        ...
        {
            split_type_list(body)
                .into_iter()
                .map(|field| {
                    let field = field.trim();
                    let (name, ty) = split_tuple_field(field)
                        .map_or((None, field), |(name, ty)| (Some(name.trim().to_string()), ty.trim()));
    ...
    {
        return Some(Type::Set {
            element: Box::new(parse_type_text(body.trim())?),
        });
    }
    if let Some(body) = name.strip_prefix('[').and_then(|value| value.strip_suffix(']'))
    {
        if let Some((key, value)) = split_collection_type(body, ':')
        {
            return Some(Type::Map {
                key: Box::new(parse_type_text(key.trim())?),
