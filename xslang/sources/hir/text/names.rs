...
pub(super) fn literal_name(literal: &Literal) -> String
{
    match literal
    {
        ...
        Literal::EnumVariant {
            ...
            variant,
            tag,
        } => format!("enum {enum_type}::{variant} tag {tag}"),
    }
}
...
pub(super) fn field_path_name(path: &FieldPath) -> String
{
    std::iter::once(path.root.as_str())
        .chain(path.fields.iter().map(String::as_str))
        .collect::<Vec<_>>()
        .join(".")
}
...
pub(super) fn match_pattern_name(pattern: &MatchPattern) -> String
{
    match pattern
    {
        ...
        {
            format!(
                "enum_data {enum_type}::{variant} owner {owner} tag {tag} binding {} : {}",
                binding.as_deref().unwrap_or("else"),
...
pub(super) fn type_name(ty: &Type) -> String
{
    match ty
    {
        ...
        Type::Optional {
            element,
        } => format!("Optional<{}>", type_name(element)),
        Type::Result {
            success,
            error,
        ...
        Type::Reference {
            referent,
            mutable: false,
        } => format!("&{}", type_name(referent)),
        Type::Reference {
            referent,
            mutable: true,
        } => format!("&mut {}", type_name(referent)),
        Type::Array {
            element,
            length: None,
        } => format!("[{}]", type_name(element)),
        Type::Array {
            element,
            length: Some(length),
        ...
        Type::Set {
            element,
        } => format!("set [{}]", type_name(element)),
        Type::Map {
            key,
            value,
        } => format!("[{}: {}]", type_name(key), type_name(value)),
        Type::Tuple {
            fields,
        } => format!(
            "({})",
