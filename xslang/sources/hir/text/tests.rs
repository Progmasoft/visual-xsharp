...
fn roundtrips_binary_expression()
{
    ...
    assert!(matches!(&parsed.body[0], Statement::Return {
        value: Some(Expression::Binary {
            operator: BinaryOperator::LessEqual,
            ..
        }),
...
fn roundtrips_result_propagation_expression()
{
    ...
    assert!(matches!(
        &parsed.body[0],
        Statement::Expr(Expression::ResultPropagation { .. })
    ));
}
...
{
    let function = DesugaredFunction {
        name: "TryWork".to_string(),
        ...
        body: vec![DesugaredStatement::Expr(DesugaredExpression::ResultMatch {
            value: Box::new(DesugaredExpression::Local {
                name: "work".to_string(),
                span: span(),
            }),
...
{
    let diagnostics = parse_xhir_function(".xhir version 2\nfunction Main\n  signature\n    returns void\n")
        .expect_err("unsupported XHIR version must fail");
...
{
    let diagnostics =
        parse_xhir_module_symbols(".xhir version 1\nmodule App\n\ndeclarations\n  symbol Main\n    kind nope\n")
...
{
    let diagnostics = parse_xhir_function(".xhir version 1\nfunction Main\n  signature\n    returns ?\n")
...
{
    let diagnostics =
        parse_xhir_typecheck_analysis("analysis typecheck\n  diagnostic nope\n    span bad\n    message broken\n")
