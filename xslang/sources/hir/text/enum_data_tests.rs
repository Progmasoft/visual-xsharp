...
{
  let function = Function { name: "wrap".to_string(),
...
{
  let function =
    Function { name: "finish".to_string(),
               return_type: Some(Type::Named("Token".to_string())),
               locals: Vec::new(),
               body: vec![Statement::Return { value: Some(Expression::EnumData { enum_type:
                                                                                   "Token".to_string(),
  ...
  assert_eq!(parse_xhir_function(&text).expect("payload-free enum-data XHIR"),
             function);
}
...
fn malformed_enum_data_tag_is_reported()
{
  let text =
    ".xhir version 1\nfunction broken\n  signature\n    returns Value\n  .end\n  body\n    return\n      enum_data \
     Value::Number owner Value tag nope payload Int\n        value\n          literal integer 1\n      .end\n  \
     .end\n.program end\n";
