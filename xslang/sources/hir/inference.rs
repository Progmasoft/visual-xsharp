    ...
    let ty = match binding.operator
    {
        BindingOperator::Assign => binding.annotation.clone().ok_or_else(|| {
            diagnostic(
                ...
                "a declaration using '=' requires an explicit type annotation",
                binding.span,
            )
        })?,
        BindingOperator::InferAssign =>
        {
            if binding.annotation.is_some()
            {
                return Err(diagnostic(
                    DiagnosticCode::InferenceBindingHasAnnotation,
            ...
            let inferred = infer_expression_type(initializer, locals).ok_or_else(|| {
                diagnostic(
                    ...
                    "the initializer does not provide a type for this ':=' declaration",
                    binding.span,
                )
            })?;
...
pub fn infer_expression_type(expression: &Expression, locals: &[Local]) -> Option<Type>
{
    match expression
    {
        ...
        Expression::Set {
            ...
            element: Box::new(homogeneous_expression_type(elements, locals)?),
        }),
        Expression::Map {
            entries, ..
        } =>
        {
            ...
            Some(Type::Map {
                key: Box::new(homogeneous_expression_refs_type(&keys, locals)?),
        ...
        Expression::Assign {
            value, ..
        } => infer_expression_type(value, locals),
...
) -> Option<Type>
{
    if operator == BinaryOperator::Coalesce
    {
        ...
        let Type::Optional {
            element,
        } = infer_expression_type(left, locals)?
        else
        {
            return None;
        };
...
mod tests
{
    ...
    {
        assert_eq!(
            resolve_binding(&inferred(Literal::Integer("26".to_string())), &[])
                .unwrap()
                .ty,
            Type::Primitive(PrimitiveType::Int)
        );
        assert_eq!(
            resolve_binding(&inferred(Literal::Float("3.14".to_string())), &[])
                .unwrap()
                .ty,
            Type::Primitive(PrimitiveType::Float)
        );
        assert_eq!(
            resolve_binding(&inferred(Literal::Char(u32::from(b'A'))), &[])
                .unwrap()
                .ty,
            Type::Primitive(PrimitiveType::Char)
        );
        assert_eq!(
            resolve_binding(&inferred(Literal::String("A".to_string())), &[])
                .unwrap()
                .ty,
            Type::Reference {
                referent: Box::new(Type::Primitive(PrimitiveType::Str)),
    ...
    fn infer_assign_requires_an_initializer()
    {
        ...
        assert_eq!(
            resolve_binding(&binding, &[]).unwrap_err().code,
            DiagnosticCode::MissingInferenceInitializer
        );
    }
    ...
    {
        assert_eq!(
            resolve_binding(&inferred(Literal::None), &[]).unwrap_err().code,
            DiagnosticCode::CannotInferType
        );
    }
    ...
    fn optional_local_cannot_be_inferred()
    {
        ...
        let binding = Binding {
            initializer: Some(Expression::Local {
                name: "source".to_string(),
                span: span(),
            }),
            ..inferred(Literal::Bool(true))
        ...
        assert_eq!(
            resolve_binding(&binding, &[optional]).unwrap_err().code,
            DiagnosticCode::OptionalTypeRequiresAnnotation
        );
    }
    ...
    fn explicit_optional_annotation_does_not_infer()
    {
        ...
        assert_eq!(
            resolve_binding(&binding, &[]).unwrap().ty,
    ...
    fn infers_result_propagation_success_type()
    {
        ...
        assert_eq!(
            resolve_binding(&binding, &[result]).unwrap().ty,
