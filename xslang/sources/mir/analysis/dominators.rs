...
fn solve_dominators(cfg: &ControlFlowGraph,
                    order: &[BlockId],
                    indices: &HashMap<BlockId, usize>,
...
fn compute_immediate(entry: Option<BlockId>,
                     order: &[BlockId],
                     indices: &HashMap<BlockId, usize>,
                     dominators: &[BitSet])
                     -> IndexMap<BlockId, Option<BlockId>>
{
  order.iter()
       ...
       .map(|block| {
         ...
         let immediate = dominators[block_index].iter()
...
fn compute_children(order: &[BlockId],
                    function_order: &[BlockId],
                    immediate: &IndexMap<BlockId, Option<BlockId>>)
                    -> IndexMap<BlockId, Vec<BlockId>>
{
  let positions = function_order.iter()
                                .enumerate()
                                .map(|(index, block)| (*block, index))
                                .collect::<HashMap<_, _>>();
  let mut children = order.iter()
                          .copied()
...
fn compute_frontiers(cfg: &ControlFlowGraph,
                     order: &[BlockId],
                     immediate: &IndexMap<BlockId, Option<BlockId>>)
                     -> IndexMap<BlockId, Vec<BlockId>>
{
