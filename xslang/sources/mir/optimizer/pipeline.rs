...
impl Default for OptimizationPipeline
{
  fn default() -> Self
  {
    Self { level: OptimizationLevel::Aggressive,
           iteration_limit: 8 }
  }
}
...
impl OptimizationPipeline
{
  ...
  pub fn run(self, function: Function) -> Result<OptimizedFunction, OptimizationError>
  {
    ...
    {
      OptimizationLevel::None => OptimizedFunction { function,
                                                     reports: Vec::new() },
  ...
  fn run_aggressive(self, mut function: Function) -> OptimizedFunction
  {
    ...
    OptimizedFunction { function: optimized.function,
                        reports }
  }
}
...
fn simplify_same_target_branches(function: &mut Function) -> usize
{
  ...
  {
    let Some(Terminator::BranchIf { then_block,
                                    else_block,
                                    .. }) = block.terminator
    else
    {
  ...
  loop
  {
    ...
    for block in &mut function.blocks
    {
      ...
      for (index, statement) in block.statements.drain(..).enumerate()
      {
        ...
        let all_definitions_dead =
          !effects.definitions().is_empty() && effects.definitions().iter().all(|local| !live_after.contains(local));
...
{
  let parameter_locals = function.parameters
                                 .iter()
...
fn canonicalize_block_order(function: &mut Function) -> usize
{
  ...
  let positions = order.iter()
                       .enumerate()
                       .map(|(index, block)| (*block, index))
                       .collect::<HashMap<BlockId, usize>>();
  let already_canonical = function.blocks
                                  .iter()
                                  .enumerate()
...
mod tests
{
  ...
  {
    let function = Function { name: "none".to_string(),
                              parameters: Vec::new(),
                              return_type: Type::VOID,
                              locals: vec![local(0, Type::I64)],
                              blocks: vec![block(0,
                                                 vec![Statement::ConstI64 { local: LocalId(0),
                                                                            value: 7,
                                                                            span: span() }],
                                                 Terminator::Return(None))] };
    let result = OptimizationPipeline::new(OptimizationLevel::None).run(function)
                                                                   .expect("valid MIR");
  ...
  {
    let function = Function { name: "dead".to_string(),
                              parameters: Vec::new(),
                              return_type: Type::VOID,
  ...
  {
    let function = Function { name: "effects".to_string(),
                              parameters: Vec::new(),
                              return_type: Type::VOID,
                              locals: vec![local(0, Type::I64),
                                           local(1, Type::I64),
                              ...
                              blocks: vec![block(0,
                                                 vec![Statement::ConstI64 { local: LocalId(0),
                                                      ...
                                                      Statement::BinaryI64 { operation:
                                                                               crate::xlil::I64BinaryOperation::Div,
                                                                             result: LocalId(2),
                                                                             left: LocalId(0),
                                                                             right: LocalId(1),
                                                                             span: span() },
                                                      Statement::Call { result: Some(LocalId(3)),
    ...
    assert!(result.function.blocks[0].statements
                                     .iter()
  ...
  {
    let function = Function { name: "same_target".to_string(),
                              parameters: Vec::new(),
                              return_type: Type::VOID,
                              locals: vec![local(0, Type::BOOL)],
                              blocks: vec![block(0, Vec::new(), Terminator::BranchIf { condition: LocalId(0),
  ...
  {
    let function = Function { name: "order".to_string(),
                              parameters: Vec::new(),
                              return_type: Type::VOID,
                              locals: vec![local(0, Type::BOOL)],
                              blocks: vec![block(0, Vec::new(), Terminator::BranchIf { condition: LocalId(0),
