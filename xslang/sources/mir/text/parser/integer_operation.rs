...
pub(super) fn is_integer_operation(name: &str) -> bool
{
    ...
    IntegerBinaryOperation::parse_text_stem(operation).is_some() &&
        type_from_name(value_type).is_some_and(|value_type| {
            value_type.is_integer() && !matches!(value_type, crate::xlil::Type::I32 | crate::xlil::Type::I64)
        })
}
...
impl Parser<'_>
{
    ...
    {
        let (operation, value_type) = instruction
            .split_once('.')
            .expect("guarded integer instruction has a type");
        let operation =
            IntegerBinaryOperation::parse_text_stem(operation).expect("guarded integer operation must parse");
