...
fn lowers_module_and_function_signature()
{
    ...
    let module = lower_declarations(&SyntaxTree {
        root: 0,
        nodes,
    })
    .expect("signature module");
...
fn preserves_enum_data_identity_and_variant_payloads()
{
    ...
    assert_eq!(
        declaration.variants[0].payload,
...
fn canonicalizes_explicit_string_without_changing_literal_inference()
{
    let mut nodes = vec![
        syntax(FILE, "", None, vec![1]),
        syntax(DECL_FUNCTION, "fn keep(value: String) -> String", Some(0), vec![
            2, 3, 6,
        ]),
    ...
    let inferred = crate::hir::inference::resolve_binding(
        &crate::hir::inference::Binding {
            ...
            initializer: Some(Expression::Literal {
                literal: Literal::String("Leitwolf".to_string()),
    ...
    assert_eq!(inferred.ty, Type::Reference {
...
fn preserves_borrowed_str_and_rejects_implicit_owned_string_construction()
{
    ...
    assert_eq!(borrowed.ty, Type::Reference {
        referent: Box::new(Type::Primitive(PrimitiveType::Str)),
        mutable: false
    });
    let Statement::Let {
    ...
    let checked = module.functions[0]
        .as_type_checked_input()
...
fn lowers_long_return_body_for_hir_to_mir()
{
    ...
    assert!(
        crate::hir::type_check::TypeChecker::new()
            .check_function(&hir)
            .is_empty()
    );
    let mir = crate::hir::mir_lowering::HirToMirLowerer::new()
        .lower_function(&hir)
        .expect("MIR body");
    assert!(matches!(mir.blocks[0].statements[0], crate::mir::Statement::ConstI32 {
        value: 7,
        ..
    }));
}
...
{
    let array = SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(TYPE_ARRAY, "[Int]", None, vec![1]),
            syntax(TYPE_NAMED, "Int", Some(0), vec![2]),
            syntax(PATH, "Int", Some(1), vec![3]),
            syntax(IDENTIFIER, "Int", Some(2), vec![]),
        ],
    };
    let fixed = SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(TYPE_FIXED_ARRAY, "[Long; 4]", None, vec![1, 4]),
    ...
    let map = SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(TYPE_MAP, "[String: Optional<Int>]", None, vec![1, 4]),
    ...
    assert_eq!(lower_type(&array, &array.nodes[0]), declarations::TypeRef::Array {
        element: Box::new(declarations::TypeRef::Primitive(PrimitiveType::Int)),
        length: None
    });
    assert_eq!(lower_type(&fixed, &fixed.nodes[0]), declarations::TypeRef::Array {
        element: Box::new(declarations::TypeRef::Primitive(PrimitiveType::Long)),
        length: Some(4)
    });
    assert_eq!(lower_type(&map, &map.nodes[0]), declarations::TypeRef::Map {
        key: Box::new(declarations::TypeRef::Primitive(PrimitiveType::String)),
        value: Box::new(declarations::TypeRef::Named("Optional<Int>".to_string()))
    });
}
...
fn resolves_fixed_array_members_to_canonical_hir()
{
    let tree = SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(EXPR_MEMBER_ACCESS, "values.count", None, vec![1, 4]),
    ...
    let array_type = Type::Array {
        element: Box::new(Type::Primitive(PrimitiveType::Long)),
        length: Some(3),
    };
    let locals = HashMap::from([("values".to_string(), array_type)]);

    assert_eq!(
        collection::array_member_type(&tree, &tree.nodes[0], &context, &locals),
    ...
    assert_eq!(
        collection::array_member_type(&first_tree, &first_tree.nodes[0], &context, &locals),
        None
    );
}
...
fn point_nominal() -> declarations::NominalType
{
    declarations::NominalType {
        ...
        fields: vec![
            declarations::Field {
                name: "x".to_string(),
                ty: declarations::TypeRef::Primitive(PrimitiveType::Long),
                mutable: true,
                span: SourceSpan {
                    file_id: 1,
                    start_offset: 0,
            ...
            declarations::Field {
                ...
                ty: declarations::TypeRef::Primitive(PrimitiveType::Bool),
                mutable: false,
                span: SourceSpan {
                    file_id: 1,
                    start_offset: 0,
...
fn optional_member_tree(member: &str) -> SyntaxTree
{
    SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(EXPR_OPTIONAL_MEMBER_ACCESS, &format!("point?.{member}"), None, vec![
                1, 4,
            ]),
...
fn resolves_optional_member_type_from_the_wrapped_nominal()
{
    ...
    let locals = HashMap::from([("point".to_string(), Type::Optional {
        element: Box::new(Type::Named("Point".to_string())),
    })]);
    assert_eq!(
        nominal::optional_member_type(&tree, &tree.nodes[0], &context, &locals),
        Some(Type::Optional {
            element: Box::new(Type::Primitive(PrimitiveType::Long))
        })
    );
    ...
    assert_eq!(
        nominal::optional_member_type(&visible, &visible.nodes[0], &context, &locals),
        Some(Type::Optional {
            element: Box::new(Type::Primitive(PrimitiveType::Bool))
        })
    );
}
...
fn lowers_optional_member_to_explicit_typed_hir()
{
    let tree = optional_member_tree("x");
    let context = point_context();
    let receiver_type = Type::Optional {
        element: Box::new(Type::Named("Point".to_string())),
    };
    ...
    let lowered = lower_expression(&tree, &tree.nodes[0], &context, &locals, Some(&result_type))
        .expect("valid optional member should lower");
...
fn optional_member_rejects_non_optional_and_unknown_receivers()
{
    ...
    assert_eq!(
        nominal::optional_member_type(&tree, &tree.nodes[0], &context, &plain),
        None
    );

    let optional_long = HashMap::from([("point".to_string(), Type::Optional {
        element: Box::new(Type::Primitive(PrimitiveType::Long)),
    })]);
    assert_eq!(
        nominal::optional_member_type(&tree, &tree.nodes[0], &context, &optional_long),
        None
    );

    let unknown = HashMap::from([("point".to_string(), Type::Optional {
        element: Box::new(Type::Named("Unknown".to_string())),
    })]);
    assert_eq!(
        nominal::optional_member_type(&tree, &tree.nodes[0], &context, &unknown),
        None
    );
}
...
fn optional_member_rejects_unknown_fields_and_wrong_context_types()
{
    ...
    let wrong_expected = Type::Optional {
        element: Box::new(Type::Primitive(PrimitiveType::Bool)),
    };
...
fn lowers_postfix_optional_unwrap_from_syntax_packet()
{
    let tree = SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(EXPR_OPTIONAL_FORGIVING, "value!", None, vec![1]),
    ...
    let context = point_context();
    let locals = HashMap::from([("value".to_string(), Type::Optional {
        element: Box::new(Type::Primitive(PrimitiveType::Long)),
    })]);
...
fn lowers_optional_coalescing_assignment_with_implicit_some()
{
    ...
    let context = point_context();
    let optional_long = Type::Optional {
        element: Box::new(Type::Primitive(PrimitiveType::Long)),
    };
    let locals = HashMap::from([("value".to_string(), optional_long.clone())]);
    assert!(
        matches!(lower_expression(&tree, &tree.nodes[0], &context, &locals, Some(&optional_long)),
...
{
    Type::Result {
        success: Box::new(Type::Primitive(PrimitiveType::Long)),
...
fn lowers_result_generic_to_a_structured_type()
{
    ...
    assert_eq!(lower_type(&tree, &tree.nodes[0]), declarations::TypeRef::Result {
        success: Box::new(declarations::TypeRef::Primitive(PrimitiveType::Long)),
        error: Box::new(declarations::TypeRef::Primitive(PrimitiveType::Bool))
    });
    assert_eq!(
        checked_type(&lower_type(&tree, &tree.nodes[0])),
        Some(Type::Result {
            success: Box::new(Type::Primitive(PrimitiveType::Long)),
...
fn lowers_canonical_result_path_and_unit_shorthand()
{
    ...
    assert_eq!(
        lower_type(&canonical, &canonical.nodes[0]),
        declarations::TypeRef::Result {
            success: Box::new(declarations::TypeRef::Primitive(PrimitiveType::Long)),
    ...
    assert_eq!(
        lower_type(&shorthand, &shorthand.nodes[0]),
        declarations::TypeRef::Result {
            success: Box::new(declarations::TypeRef::Unit),
...
fn result_constructor_tree(name: &str, literal_text: &str, token_kind: u32) -> SyntaxTree
{
    ...
    SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(EXPR_CALL, &format!("{name}({literal_text})"), None, vec![1, 4]),
...
fn lowers_ok_and_error_calls_against_result_context()
{
    ...
    for name in ["Ok", "Error"]
    {
        ...
        .expect("Result constructor should lower");
        assert!(matches!(lowered,
                     Expression::Call {
                       function,
                       arguments,
...
fn result_constructor_rejects_wrong_payload_and_non_result_context()
{
    ...
    let lowered = lower_expression(
        ...
        &empty_context(),
        &HashMap::new(),
        Some(&result_long_type()),
    )
    .expect("syntax lowering preserves the mismatched payload for type diagnostics");
    let function = crate::hir::type_check::Function {
...
fn propagation_tree() -> SyntaxTree
{
    SyntaxTree {
        root: 0,
        nodes: vec![
            syntax(EXPR_RESULT_PROPAGATION, "work@", None, vec![1]),
...
fn lowers_postfix_result_propagation_to_typed_hir()
{
    ...
    .expect("Result propagation should lower");
    assert!(matches!(lowered,
    ...
    assert_eq!(
        expression_type(&tree, &tree.nodes[0], &empty_context(), &locals),
        Some(Type::Primitive(PrimitiveType::Long))
    );
}
