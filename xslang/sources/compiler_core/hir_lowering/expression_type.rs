...
) -> Option<Type>
{
    match value.kind
    {
        ...
        EXPR_LITERAL if value.token_kind == TOKEN_STRING => Some(Type::Reference {
            referent: Box::new(Type::Primitive(PrimitiveType::Str)),
            mutable: false,
        }),
        EXPR_IDENTIFIER if value.text == "nil" => None,
        EXPR_IDENTIFIER => nominal::enum_variant_type(tree, value, context)
            .or_else(|| nominal::enum_data_variant_type(tree, value, context))
            .or_else(|| locals.get(&path_text(tree, value)).cloned()),
        EXPR_MEMBER_ACCESS => tuple::tuple_element_type(tree, value, context, locals)
            .or_else(|| collection::array_member_type(tree, value, context, locals))
            .or_else(|| nominal::member_type(tree, value, context, locals)),
        ...
        EXPR_METHOD_CALL =>
        {
            call::lower_method_call(tree, value, context, locals, None, span(value)?).and_then(|expression| {
                match expression
                {
                    Expression::Call {
                        return_type, ..
