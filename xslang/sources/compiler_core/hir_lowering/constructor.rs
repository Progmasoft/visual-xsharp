...
) -> Result<declarations::Function, LoweringError>
{
    let parameters = value
        ...
        .collect::<Result<_, _>>()?;
    Ok(declarations::Function {
        name: format!("xs$ctor${owner}${ordinal}"),
        parameters,
...
fn collect_required_fields(
    definition: &declarations::NominalType,
    context: &LoweringContext,
    prefix: &[String],
    ...
    for statement in statements
    {
        match statement
        {
            ...
            } =>
            {
                ...
                let else_assigned = else_block.as_ref().map_or_else(
                    || assigned.clone(),
...
) -> Option<&'a CallSignature>
{
    ...
    let argument_types = call.children[1..]
        .iter()
...
pub(super) fn new_expression_type(
    tree: &SyntaxTree,
    call: &SyntaxNode,
    context: &LoweringContext,
    locals: &HashMap<String, Type>,
...
) -> Option<Expression>
{
    ...
    let arguments = call.children[1..]
        ...
        .map(|(index, parameter)| lower_expression(tree, tree.nodes.get(*index)?, context, locals, Some(parameter)))
        .collect::<Option<Vec<_>>>()?;
