...
pub(super) fn default_array_initializer(value_type: &Type, span: Span) -> Option<Expression>
{
    ...
    let elements = (0..count)
        .map(|_| default_element(element, span))
...
pub(super) fn refine_local_type(value_type: &Type, initializer: Option<&SyntaxNode>) -> Type
{
    match (value_type, initializer)
    {
        (
            ...
            Some(value),
        ) if is_set_literal(value.kind) => Type::Set {
...
) -> Option<Statement>
{
    ...
    let index = super::lower_expression(
        tree,
    ...
    let value = super::lower_expression(
        tree,
...
pub(super) fn array_member_type(
    tree: &SyntaxTree,
    member: &SyntaxNode,
    context: &LoweringContext,
    locals: &HashMap<String, Type>,
) -> Option<Type>
{
    ...
    let Type::Array {
        element,
        length,
    } = super::expression_type(tree, receiver, context, locals)?
    else
    {
        return None;
    };
...
pub(super) fn lower_array_member(
    tree: &SyntaxTree,
    member: &SyntaxNode,
    context: &LoweringContext,
    locals: &HashMap<String, Type>,
...
) -> Option<Expression>
{
    ...
    match name.as_str()
    {
        "count" | "capacity" | "end_index" => match length
        {
            ...
            None => Some(Expression::ArrayLength {
                collection: Box::new(super::lower_expression(
                    tree,
                    receiver_node,
                    context,
                    locals,
        ...
        {
            Some(length) => Some(Expression::Literal {
                literal: Literal::Bool(*length == 0),
                span,
            }),
            None => Some(Expression::Binary {
        ...
        "first" | "last" =>
        {
            ...
            let index = match (name.as_str(), length)
            {
                ...
                ("last", None) => Expression::Binary {
                    ...
                    left: Box::new(Expression::ArrayLength {
                        collection: Box::new(receiver.clone()),
                        span,
                    }),
...
{
    i64::try_from(value).ok().map(|value| Expression::Literal {
...
) -> Option<Type>
{
    match value.kind
    {
        ...
        EXPR_SET_LITERAL => homogeneous_type(tree, value, context, locals).map(|element| Type::Set {
            element: Box::new(element),
        }),
...
) -> Option<Expression>
{
    ...
    let mut elements = value.children[..retained]
        .iter()
...
) -> Option<Expression>
{
    ...
    let Type::Map {
        key,
        value,
    } = expected_type.or(inferred.as_ref())?
    else
    {
        return None;
    };
    ...
    node.children
        ...
        .all(|index| {
            ...
            super::expression_type(tree, &tree.nodes[entry.children[0]], context, locals).as_ref() == Some(&key) &&
                super::expression_type(tree, &tree.nodes[entry.children[1]], context, locals).as_ref() ==
                    Some(&value)
        })
        .then(|| Type::Map {
            key: Box::new(key),
