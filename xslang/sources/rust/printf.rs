...
fn parse_spec(characters: &[char], cursor: &mut usize) -> io::Result<FormatSpec>
{
  ...
  spec.conversion = *characters.get(*cursor)
                               .ok_or_else(|| invalid("printf conversion is missing"))?;
  *cursor += 1;
  if !matches!(spec.conversion,
...
fn format_value(spec: FormatSpec, value: &PrintfValue) -> io::Result<String>
{
  let body = match (spec.conversion, value)
  {
    ...
    ('f' | 'F', PrintfValue::Float(value)) => format!("{value:.precision$}", precision = spec.precision.unwrap_or(6)),
    ('e', PrintfValue::Float(value)) => format!("{value:.precision$e}", precision = spec.precision.unwrap_or(6)),
    ('E', PrintfValue::Float(value)) => format!("{value:.precision$E}", precision = spec.precision.unwrap_or(6)),
    ('g' | 'G', PrintfValue::Float(value)) => format!("{value:.precision$}", precision = spec.precision.unwrap_or(6)),
    ('s', PrintfValue::Text(value)) => truncate(value, spec.precision),
    ('c', PrintfValue::Character(value)) => value.to_string(),
    ('c', PrintfValue::Unsigned(value)) => u32::try_from(*value).ok()
...
mod tests
{
  ...
  {
    let arguments = PrintfArguments::new("name=%s count=%+05d hex=%#x ratio=%.2f %%",
                                         vec![PrintfValue::Text("Alpha".to_owned()),
                                              PrintfValue::Signed(7),
                                              PrintfValue::Unsigned(255),
                                              PrintfValue::Float(1.25)]);
    assert_eq!(arguments.render().unwrap(),
  ...
  fn rejects_argument_count_and_type_mismatches()
  {
    ...
    assert!(PrintfArguments::new("%s", vec![PrintfValue::Signed(1)]).render()
                                                                    .is_err());
    assert!(PrintfArguments::new("plain", vec![PrintfValue::Signed(1)]).render()
                                                                       .is_err());
  }
}
