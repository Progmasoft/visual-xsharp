...
impl Config
{
  pub(crate) fn parse(tokens: TokenStream) -> syn::Result<Self>
  {
    ...
    for value in values
    {
      ...
      {
        return Err(Error::new_spanned(value.path, "xlil_create option names cannot be qualified"));
      };
      if !seen.insert(name.clone())
      {
        return Err(Error::new_spanned(value.path, format!("duplicate xlil_create option '{name}'")));
      }
      ...
      match name.as_str()
      {
        "module" =>
        {
          if text.is_empty()
          {
            return Err(Error::new_spanned(value.value, "xlil_create module name cannot be empty"));
          }
        ...
        _ =>
        {
          return Err(Error::new_spanned(value.path,
...
{
  let Expr::Lit(ExprLit { lit: Lit::Str(value), .. }) = expression
  else
  {
    return Err(Error::new_spanned(expression, "xlil_create option values must be string literals"));
  };
  Ok(value.value())
}
...
fn identifier(expression: &Expr, value: &str) -> syn::Result<Ident>
{
  syn::parse_str::<Ident>(value).map_err(|_| {
                                  Error::new_spanned(expression, "xlil_create producer names must be Rust identifiers")
                                })
}
