...
fn validate_signature(signature: &Signature) -> syn::Result<()>
{
        ...
        signature.abi.is_some()
    {
        return Err(Error::new_spanned(
            signature,
            "xlil_create requires a synchronous, safe Rust function",
        ));
    }
    if !signature.generics.params.is_empty() || signature.generics.where_clause.is_some()
    {
        return Err(Error::new_spanned(
            &signature.generics,
            "xlil_create does not yet lower generics",
        ));
    }
    if signature.variadic.is_some()
    {
        return Err(Error::new_spanned(
            signature,
            "xlil_create does not lower variadic functions",
        ));
    }
    Ok(())
}
    ...
    signature
        ...
        .map(|(index, argument)| {
            ...
            else
            {
                return Err(Error::new_spanned(
                    argument,
                    "xlil_create can only be applied to free functions",
                ));
            };
            ...
            if pattern.by_ref.is_some() || pattern.mutability.is_some() || pattern.subpat.is_some()
            {
                return Err(Error::new_spanned(
                    pattern,
                    "xlil_create parameters cannot use ref, mut, or subpatterns",
                ));
            }
            Ok(Parameter {
                source: pattern.ident.clone(),
