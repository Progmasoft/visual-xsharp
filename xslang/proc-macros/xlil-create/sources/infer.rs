...
pub(crate) fn infer_expression_type(expression: &Expr, bindings: &HashMap<String, Binding>) -> Option<ValueType>
{
  match strip_groups(expression)
  {
    ...
    Expr::Lit(ExprLit { lit: Lit::Float(value), .. }) => match value.suffix()
    {
    ...
    Expr::If(branch) => infer_block_type(&branch.then_branch, bindings).or_else(|| {
                                                                         branch.else_branch
                                                                               .as_ref()
                                                                               .and_then(|(_, value)| {
                                                                                 infer_expression_type(value, bindings)
                                                                               })
                                                                       }),
...
mod tests
{
  ...
  fn literal_and_unary_types_are_inferred()
  {
    ...
    assert_eq!(infer_expression_type(&expression(quote!(true)), &bindings),
               Some(ValueType::Bool));
    assert_eq!(infer_expression_type(&expression(quote!(1.5f32)), &bindings),
               Some(ValueType::F32));
    assert_eq!(infer_expression_type(&expression(quote!(-1.5f64)), &bindings),
               Some(ValueType::F64));
    assert_eq!(infer_expression_type(&expression(quote!(!false)), &bindings),
               Some(ValueType::Bool));
  }
  ...
  fn registered_binding_type_is_inferred()
  {
    ...
    bindings.insert("value".into(), Binding { binding: format_ident!("generated"),
                                              value_type: ValueType::I32 });
    assert_eq!(infer_expression_type(&expression(quote!(value)), &bindings),
               Some(ValueType::I32));
    assert_eq!(infer_expression_type(&expression(quote!(value + 1)), &bindings),
               Some(ValueType::I32));
  }
  ...
  {
    let bindings = HashMap::new();
    assert_eq!(infer_expression_type(&expression(quote!(1 < 2)), &bindings),
               Some(ValueType::Bool));
    assert_eq!(infer_expression_type(&expression(quote!(true && false)), &bindings),
               Some(ValueType::Bool));
  }
