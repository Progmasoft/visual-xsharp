...
impl Lowerer
{
  ...
  fn lower_block_terminal(&mut self,
                          block: &Block,
                          ...
                          -> syn::Result<TokenStream>
  {
    ...
    else
    {
      ...
      return Err(Error::new_spanned(block, "value-producing xlil_create function has an empty body"));
    };
    ...
    for statement in prefix
    {
      ...
      {
        return Err(Error::new_spanned(statement,
  ...
  fn lower_local(&mut self, local: &Local) -> syn::Result<TokenStream>
  {
    ...
    {
      return Err(Error::new_spanned(local, "xlil_create let bindings require an initializer"));
    };
    if initializer.diverge.is_some()
    {
      return Err(Error::new_spanned(local, "xlil_create does not lower let-else bindings"));
    }
    ...
    self.bindings
        .insert(identifier.to_string(), Binding { binding: local_binding.clone(),
  ...
  fn lower_value(&mut self, expression: &Expr, expected: Option<ValueType>) -> syn::Result<Value>
  {
    match strip_groups(expression)
    {
      ...
      other => Err(Error::new_spanned(other, "xlil_create does not yet lower this Rust expression")),
    }
  }
  ...
  fn lower_path(&self, path: &ExprPath) -> syn::Result<Value>
  {
    ...
    {
      return Err(Error::new_spanned(path, "xlil_create could not resolve this parameter or local binding"));
    };
  ...
  fn lower_literal(&mut self, literal: &ExprLit, expected: Option<ValueType>) -> syn::Result<Value>
  {
    ...
    match &literal.lit
    {
      ...
      {
        let value_type =
          expected.filter(|candidate| candidate.supports_float_arithmetic())
      ...
      _ => Err(Error::new_spanned(literal,
  ...
  fn lower_binary(&mut self, binary: &ExprBinary, expected: Option<ValueType>) -> syn::Result<Value>
  {
    ...
    {
      return Err(Error::new_spanned(binary.op,
  ...
  fn lower_unary(&mut self, unary: &ExprUnary, expected: Option<ValueType>) -> syn::Result<Value>
  {
    match unary.op
    {
      ...
      UnOp::Neg(_) =>
      {
        let value_type = infer_expression_type(&unary.expr, &self.bindings).or(expected)
                                                                           .filter(|candidate| {
                                                                             candidate.is_integer() ||
                                                                             candidate.supports_float_arithmetic()
                                                                           })
                                                                           .ok_or_else(|| {
                                                                             Error::new_spanned(unary,
                                                                                                "xlil_create cannot \
                                                                                                 infer unary negation \
                                                                                                 type")
                                                                           })?;
  ...
  fn lower_block_value(&mut self, block: &Block, expected: Option<ValueType>) -> syn::Result<Value>
  {
    ...
    let result = (|| {
      ...
      for statement in prefix
      {
        ...
        {
          return Err(Error::new_spanned(statement, "xlil_create value blocks permit only immutable let bindings"));
        };
        statements.extend(self.lower_local(local)?);
      }
      let expression = statement_expression(last)?;
  ...
  fn lower_if_value(&mut self, branch: &ExprIf, expected: Option<ValueType>) -> syn::Result<Value>
  {
    ...
    let inferred = infer_block_type(&branch.then_branch, &self.bindings).or_else(|| {
                                                                          infer_expression_type(else_expression,
                                                                                                &self.bindings)
                                                                        });
    let result_type =
    ...
    {
      return Err(Error::new_spanned(&branch.then_branch,
                                    format!("xlil_create type mismatch: expected {result_type:?}, found {:?}",
  ...
  {
    infer_expression_type(&binary.left, &self.bindings).or_else(|| infer_expression_type(&binary.right, &self.bindings))
                                                       .or_else(|| {
...
fn statement_expression(statement: &Stmt) -> syn::Result<&Expr>
{
  match statement
  {
    ...
    _ => Err(Error::new_spanned(statement,
...
fn local_pattern(pattern: &Pat) -> syn::Result<(Ident, Option<ValueType>)>
{
  match pattern
  {
    ...
    Pat::Type(typed) =>
    {
      ...
      {
        return Err(Error::new_spanned(pattern, "xlil_create typed let requires an identifier pattern"));
      };
      if identifier.by_ref.is_some() || identifier.mutability.is_some() || identifier.subpat.is_some()
      {
        return Err(Error::new_spanned(pattern, "xlil_create let bindings must be immutable identifiers"));
      }
      Ok((identifier.ident.clone(), Some(ValueType::parse(&typed.ty)?)))
    }
    _ => Err(Error::new_spanned(pattern, "xlil_create let bindings must be immutable identifiers")),
  }
}
