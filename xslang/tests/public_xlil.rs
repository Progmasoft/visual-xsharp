...
fn public_rust_api_builds_verifies_and_roundtrips_a_module()
{
    ...
    builder
        .declare_function("Import", Type::I32, vec![Type::I32])
        .expect("public API should add declarations");
    builder
        .begin_function("main", Type::I32, Vec::new())
    ...
    let result = builder
        .call("Import", vec![argument], Type::I32)
...
fn public_builder_reports_invalid_state_and_duplicate_functions()
{
    ...
    assert_eq!(
        builder.declare_function("main", Type::I32, Vec::new()),
...
fn checked_calls_derive_the_registered_signature()
{
    ...
    builder
        .declare_function("Identity", Type::I32, vec![Type::I32])
        .unwrap();
    builder.begin_function("main", Type::I32, Vec::new()).unwrap();
...
fn checked_calls_report_signature_mismatches_early()
{
    ...
    assert!(matches!(
        builder.call_checked("Missing", vec![]),
        Err(BuildError::UnknownFunction(_))
    ));
    assert!(matches!(
        builder.call_checked("Import", vec![]),
        Err(BuildError::ArgumentCount { .. })
    ));
    assert!(matches!(
        builder.call_checked("Import", vec![argument]),
        Err(BuildError::ArgumentType { .. })
    ));
    assert!(matches!(
        builder.call_void("Import", vec![argument]),
